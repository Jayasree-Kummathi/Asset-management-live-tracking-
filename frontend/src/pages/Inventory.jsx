...
import {
  ...
  Camera, ScanLine, QrCode, RefreshCw, AlertTriangle
} from 'lucide-react';
          ...
          {result ? (
            <div>
              <div style={{display:'flex',gap:16,marginBottom:20}}>
                <div style={{flex:1,padding:'16px',background:'var(--green-bg)',border:'1px solid rgba(52,211,153,.2)',borderRadius:'var(--radius)',textAlign:'center'}}>
                ...
                <div style={{flex:1,padding:'16px',background:'var(--red-bg)',border:'1px solid rgba(248,113,113,.2)',borderRadius:'var(--radius)',textAlign:'center'}}>
              ...
              {result.created.length>0 && <div style={{marginBottom:12}}><div style={{fontSize:12,color:'var(--text-muted)',marginBottom:6}}>Created:</div><div style={{display:'flex',flexWrap:'wrap',gap:6}}>{result.created.map(id=><span key={id} style={{background:'var(--green-bg)',color:'var(--green)',padding:'2px 10px',borderRadius:20,fontSize:12,fontFamily:'var(--mono)'}}>{id}</span>)}</div></div>}
              {result.failed.length>0 && <div><div style={{fontSize:12,color:'var(--text-muted)',marginBottom:6}}>Failed:</div>{result.failed.map((f,i)=><div key={i} style={{fontSize:12,color:'var(--red)',marginBottom:4}}>{f.id}: {f.reason}</div>)}</div>}
              <div style={{display:'flex',justifyContent:'flex-end',marginTop:20}}>
            ...
            <>
              <div style={{marginBottom:14}}>
                ...
                <textarea className="form-textarea" style={{minHeight:160,fontFamily:'var(--mono)',fontSize:12}}
                  value={csvText} onChange={e=>setCsvText(e.target.value)} placeholder="asset_id,serial,brand,model,..."/>
              </div>
              ...
              {preview.length>0 && (
                <div style={{marginBottom:16}}>
                  ...
                  <div style={{maxHeight:200,overflowY:'auto'}} className="table-wrap">
                    <table>
                      <thead><tr><th>Asset ID</th><th>Serial</th><th>Brand</th><th>Model</th><th>Config</th><th>Location</th></tr></thead>
                      <tbody>
                        {preview.map((r,i)=>(
                          <tr key={i}>
...
export default function Inventory() {
  const { assets, addAsset, refetch } = useApp();
  const { user } = useAuth();
  ...
  const [qrDefaultTab,  setQrDefaultTab] = useState('import'); // ✅ controls which tab opens
  ...
  // ✅ Helper: open QR modal on a specific tab
  const openQRModal = (tab) => {
    setQrDefaultTab(tab);
    setShowQRImport(true);
  };

  const filtered = assets.filter(a => {
    const ms = !search || [a.id,a.serial,a.brand,a.model,a.config].some(v=>v?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="fade-in">
        ...
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          {isAdmin && (
            <>
              ...
              {/* ✅ Import & Generate QR — opens on import tab */}
              ...
              {/* ✅ Reprint QR — opens directly on reprint tab */}
              <button className="btn btn-secondary"
      ...
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {filtered.length===0 ? (
              <tr><td colSpan={8}><div className="empty-state"><p>No assets found</p></div></td></tr>
            ) : filtered.map(a => {
              ...
              return (
                <tr key={a.id}>
                  ...
                  <td><span style={{fontFamily:'var(--mono)',fontSize:12,color:expired?'var(--red)':'var(--text-dim)'}}>{a.warrantyEnd||'—'}</span></td>
      ...
      {/* ✅ QR modal — passes defaultTab so it opens on the right tab */}
      {showQRImport && (
        <QRBulkImport
          onImported={refetch}
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                {scanFlash && (
                  <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:12,
                    color:'var(--green)',background:'var(--green-bg)',padding:'3px 10px',
                    borderRadius:20,animation:'fadeIn 0.3s ease'}}>
            ...
            <div className="modal-body">
              <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',marginBottom:18,
                background:'var(--accent-glow,rgba(99,102,241,0.08))',border:'1px solid var(--accent,#6366f1)',
              ...
              <form onSubmit={handleAdd}>
                ...
                <div className="form-grid form-grid-3">
                  <div className="form-group">
                    ...
                    <input className={`form-input${scanFlash&&form.id?' scan-filled':''}`} required value={form.id} onChange={e=>set('id',e.target.value)} placeholder="LTB-276"/>
                  </div>
                  <div className="form-group">
                    <label className="form-label">Serial Number *</label>
                    <input className={`form-input${scanFlash&&form.serial?' scan-filled':''}`} required value={form.serial} onChange={e=>set('serial',e.target.value)} placeholder="DX91"/>
                  </div>
                  <div className="form-group">
