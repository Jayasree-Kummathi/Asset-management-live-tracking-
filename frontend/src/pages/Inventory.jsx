    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)' }}>
          <span style={{ cursor: 'pointer', color: 'var(--accent)' }} onClick={onBack}>Inventory</span>
          <ChevronRight size={13} />
          <span style={{ color: 'var(--text)', fontWeight: 600, fontFamily: 'var(--mono)' }}>{asset.id}</span>
        </div>
      </div>
      ...
      {/* Warranty urgent banner */}
      {(expired || warrantyUrgent) && (
        <div style={{
          display: 'flex', alignItems: 'center', gap: 10,
          padding: '12px 16px', marginBottom: 20,
          background: expired ? 'var(--red-bg)' : 'rgba(251,191,36,.1)',
          border: `1px solid ${expired ? 'var(--red)' : '#f59e0b'}`,
          borderRadius: 'var(--radius)', color: expired ? 'var(--red)' : '#f59e0b',
          fontWeight: 600, fontSize: 13,
        }}>
          <AlertTriangle size={16} />
          {expired
            ? `Warranty expired ${Math.abs(daysLeft)} days ago`
            : `Warranty expires in ${daysLeft} day${daysLeft !== 1 ? 's' : ''} — renew soon`}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="card">
            <div className="section-title">
              <Send size={13} style={{ display: 'inline', marginRight: 6, verticalAlign: 'middle' }} />
              Allocation
            </div>
            {alloc ? (
              <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <button className="btn btn-secondary" style={{ justifyContent: 'flex-start' }} onClick={onHistory}>
                <History size={14} /> View Activity History
              </button>
              {isAdmin && (
                <button className="btn btn-secondary" style={{ justifyContent: 'flex-start' }} onClick={onEdit}>
...
function AssetEditPage({ asset, onBack, onSaved }) {
    ...
    try {
      await apiFetch(`/assets/${asset.id}`, {
        ...
        body: JSON.stringify({
          ...
          purchase_date: toDateOnly(form.purchaseDate),
          warranty_start: toDateOnly(form.warrantyStart),
          warranty_end: toDateOnly(form.warrantyEnd),
          vendor: form.vendor,
  ...
  return (
    <div className="fade-in" style={{ animation: 'slideInEdit 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
        <button className="btn btn-secondary" onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <ArrowLeft size={15} /> Back to {asset.id}
        </button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)' }}>
          <span style={{ cursor: 'pointer', color: 'var(--accent)' }} onClick={onBack}>{asset.id}</span>
          <ChevronRight size={13} />
          <span style={{ color: 'var(--text)', fontWeight: 600 }}>Edit Asset</span>
        </div>
      </div>
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="form-grid form-grid-3">
            ...
            <div className="form-group">
              ...
              <input className="form-input" required value={form.serial} onChange={e => setField('serial', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Brand *</label>
              <input className="form-input" required value={form.brand} onChange={e => setField('brand', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Model *</label>
              <input className="form-input" required value={form.model} onChange={e => setField('model', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Configuration</label>
              <input className="form-input" value={form.config} onChange={e => setField('config', e.target.value)} />
            </div>
            <div className="form-group">
          ...
          <div className="form-grid form-grid-3">
            <div className="form-group">
              ...
              <input className="form-input" value={form.processor} onChange={e => setField('processor', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">RAM</label>
              <input className="form-input" value={form.ram} onChange={e => setField('ram', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Storage</label>
              <input className="form-input" value={form.storage} onChange={e => setField('storage', e.target.value)} />
            </div>
          </div>
          ...
          <div className="form-grid form-grid-3">
            <div className="form-group">
              ...
              <input type="date" className="form-input" value={form.purchaseDate} onChange={e => setField('purchaseDate', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Warranty Start</label>
              <input type="date" className="form-input" value={form.warrantyStart} onChange={e => setField('warrantyStart', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Warranty End</label>
              <input type="date" className="form-input" value={form.warrantyEnd} onChange={e => setField('warrantyEnd', e.target.value)} />
            </div>
            <div className="form-group">
...
function AssetHistoryPage({ asset, onBack }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await apiFetch(`/audit?asset_id=${asset.id}&limit=100`);
  ...
  return (
    <div className="fade-in" style={{ animation: 'slideInHistory 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      <style>{`
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)' }}>
          <span style={{ cursor: 'pointer', color: 'var(--accent)' }} onClick={onBack}>{asset.id}</span>
          <ChevronRight size={13} />
          <span style={{ color: 'var(--text)', fontWeight: 600 }}>Activity History</span>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '24px 28px' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[1, 2, 3, 4].map(i => (
              <div key={i} style={{ height: 72, borderRadius: 8, background: 'var(--surface2)', animation: 'shimmer 1.4s infinite', animationDelay: `${i * 80}ms` }} />
            ))}
          </div>
        ) : history.length === 0 ? (
          ...
          <div className="history-timeline">
            {Object.entries(groupedHistory).map(([date, logs]) => (
              <div key={date}>
                <div className="history-group-date">
                  <span>{date}</span>
                </div>
                {logs.map((log, idx) => {
...
function BarcodeScannerModal({ onResult, onClose }) {
  ...
  const stopCamera = () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); if (streamRef.current) { streamRef.current.getTracks().forEach(t => t.stop()); streamRef.current = null; } };
  const parseScanResult = (text) => {
    if (scannedRef.current) return; scannedRef.current = true; stopCamera();
    let data = {};
    try { data = JSON.parse(text); } catch { try { const p = new URLSearchParams(text); for (const [k,v] of p.entries()) data[k]=v; if (!Object.keys(data).length) throw new Error(); } catch { data = { serial: text }; } }
    onResult(data);
  };
  ...
  const runZxing = () => { try { const cr = new window.ZXing.BrowserQRCodeReader(new Map()); cr.decodeFromVideoElement(videoRef.current).then(r => { if (r && !scannedRef.current) parseScanResult(r.getText()); }).catch(() => { if (!scannedRef.current) setError('QR scanning failed.'); }); } catch { setError('QR scanner could not start.'); } };
  return (
...
function BulkImportModal({ onClose, onImported }) {
  ...
  const parseCSV = (text) => { const lines = text.trim().split('\n').filter(l => l.trim()); if (lines.length < 2) return []; const headers = lines[0].split(',').map(h => h.trim()); return lines.slice(1).map(line => { const vals = line.split(',').map(v => v.trim()); const obj = {}; headers.forEach((h,i) => { obj[h] = vals[i]||''; }); return obj; }); };
  const handleParse = () => { setPreview(parseCSV(csvText)); setResult(null); };
  const handleImport = async () => {
    setSaving(true);
    try { const sanitized = preview.map(row => ({ ...row, purchase_date: toDateOnly(row.purchase_date), warranty_start: toDateOnly(row.warranty_start), warranty_end: toDateOnly(row.warranty_end) })); const data = await apiFetch('/assets/bulk', { method: 'POST', body: JSON.stringify({ assets: sanitized }) }); setResult(data.data); if (data.data.created.length > 0) onImported(); }
          ...
          {result ? (
            <div>
              <div style={{ display:'flex',gap:16,marginBottom:20 }}>
                <div style={{ flex:1,padding:16,background:'var(--green-bg)',border:'1px solid rgba(52,211,153,.2)',borderRadius:'var(--radius)',textAlign:'center' }}><div style={{ fontSize:28,fontWeight:800,color:'var(--green)',fontFamily:'var(--mono)' }}>{result.created.length}</div><div style={{ fontSize:13,color:'var(--text-muted)' }}>Created Successfully</div></div>
                <div style={{ flex:1,padding:16,background:'var(--red-bg)',border:'1px solid rgba(248,113,113,.2)',borderRadius:'var(--radius)',textAlign:'center' }}><div style={{ fontSize:28,fontWeight:800,color:'var(--red)',fontFamily:'var(--mono)' }}>{result.failed.length}</div><div style={{ fontSize:13,color:'var(--text-muted)' }}>Failed</div></div>
              </div>
              ...
              {preview.length > 0 && (
                <div style={{ marginBottom:16 }}>
                  ...
                  <div style={{ maxHeight:200,overflowY:'auto' }} className="table-wrap">
                    <table><thead><tr><th>Asset ID</th><th>Serial</th><th>Brand</th><th>Model</th><th>Location</th></tr></thead>
                    <tbody>{preview.map((r,i)=>(<tr key={i}><td style={{ fontFamily:'var(--mono)',fontSize:12,color:'var(--accent)' }}>{r.asset_id}</td><td style={{ fontFamily:'var(--mono)',fontSize:12 }}>{r.serial}</td><td>{r.brand}</td><td>{r.model}</td><td>{r.location}</td></tr>))}</tbody>
                  </table>
                  </div>
                </div>
              )}
...
function BulkStatusModal({ action, count, onConfirm, onClose, loading }) {
  ...
  const META = {
    repair: { label: 'Send to Repair', color: '#f59e0b', bg: 'rgba(251,191,36,.12)', icon: <Wrench size={16}/>, inputLabel: 'Issue / Reason for Repair *', inputHint: 'e.g. Screen cracked, keyboard not working' },
    scrap:  { label: 'Scrap Assets',   color: 'var(--red)', bg: 'var(--red-bg)',      icon: <Archive size={16}/>, inputLabel: 'Reason for Scrapping *',        inputHint: 'e.g. Motherboard failure — beyond economical repair' },
  };
  const m = META[action] || {};
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        <div className="modal-header"><h2 className="modal-title">{m.label}</h2><button className="btn btn-icon" onClick={onClose}><X size={16}/></button></div>
        <div className="modal-body">
          <div style={{ padding:'14px 16px', background:m.bg, border:`1px solid ${m.color}33`, borderRadius:'var(--radius)', marginBottom:20, display:'flex', gap:12, alignItems:'flex-start' }}>
            ...
            <div style={{ fontSize:13 }}>
              ...
              <div style={{ color:'var(--text-muted)', marginTop:4, fontSize:12 }}>{action === 'repair' ? 'A repair record will be created for each asset in the Repair Assets page.' : 'A scrap record will be created for each asset in the Scrap Assets page.'}</div>
            </div>
          </div>
          <div className="form-group">
            <label className="form-label">{m.inputLabel}</label>
            <textarea className="form-textarea" style={{ minHeight: 80 }} placeholder={m.inputHint} value={reason} onChange={e => setReason(e.target.value)} />
          </div>
          <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
            <button className="btn btn-secondary" onClick={onClose}>Cancel</button>
            <button className={`btn ${action === 'scrap' ? 'btn-danger' : 'btn-primary'}`} style={action === 'repair' ? { background:'#f59e0b', borderColor:'#f59e0b' } : {}} onClick={() => onConfirm(reason)} disabled={loading || !reason.trim()}>
...
export default function Inventory() {
  ...
  // Page states - now using dedicated pages instead of modals
  ...
  const listStateSnapshot = useRef({ search: '', filter: 'All' });
  ...
  const confirmBulkStatus = async (reason) => {
    ...
    try {
      ...
      if (bulkAction === 'repair') {
        await Promise.all([...selected].map(id => apiFetch('/repairs', { method: 'POST', body: JSON.stringify({ asset_id: id, issue: reason || 'Bulk marked for repair from inventory', repair_date: today }) })));
      } else if (bulkAction === 'scrap') {
        await Promise.all([...selected].map(id => apiFetch('/scraps', { method: 'POST', body: JSON.stringify({ asset_id: id, reason: reason || 'Bulk scrapped from inventory' }) })));
      }
      setBulkAction(null); clearAll(); refetch();
    } catch (err) { alert(err.message); } finally { setBulkLoading(false); }
  };

  const handleAdd = async (e) => {
    e.preventDefault(); setSaving(true);
    try { await addAsset({ ...form, purchaseDate: toDateOnly(form.purchaseDate), warrantyStart: toDateOnly(form.warrantyStart), warrantyEnd: toDateOnly(form.warrantyEnd) }); setForm(EMPTY_FORM); setShowAdd(false); }
  ...
  // Save current list state before navigating to subpage
  ...
  const handleHistoryAsset = (asset) => {
    listStateSnapshot.current = { search, filter };
  ...
  // ── Render dedicated pages if active ──
    ...
    return (
      <>
        <AssetDetailPage
          ...
          onEdit={() => handleEditAsset(viewAsset)}
          onHistory={() => handleHistoryAsset(viewAsset)}
          isAdmin={isAdmin}
          navigate={navigate}
        />
        <style>{`
  ...
  return (
    <div className="fade-in">
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={a.id} className={isSelected ? 'row-selected' : ''}>
                  <td><button className="checkbox-btn" onClick={() => toggleSelect(a.id)}>{isSelected ? <CheckSquare size={15} style={{ color:'var(--accent)' }}/> : <Square size={15} style={{ color:'var(--text-muted)' }}/>}</button></td>
                  ...
                  <td>
                    <div style={{ display:'flex',gap:5,flexWrap:'wrap' }}>
                      ...
                      <button className="btn btn-sm btn-secondary" onClick={() => handleViewAsset(a)}><Eye size={12}/> View</button>
                      <button className="btn btn-sm btn-secondary" onClick={() => handleHistoryAsset(a)} title="History"><History size={12}/></button>
                      {isAdmin && (
                        <>
                          <button className="btn btn-sm btn-secondary" onClick={() => handleEditAsset(a)} title="Edit"><Edit2 size={12}/></button>
      ...
      {bulkAction && <BulkStatusModal action={bulkAction} count={selected.size} onConfirm={confirmBulkStatus} onClose={() => setBulkAction(null)} loading={bulkLoading}/>}
          ...
          <div className="modal" style={{ maxWidth:420 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div style={{ display:'flex',gap:10,justifyContent:'flex-end' }}>
                ...
                <button className="btn btn-danger" onClick={handleDelete} disabled={deleting||deleteConf.status==='Allocated'}><Trash2 size={14}/> {deleting?'Deleting…':'Delete Asset'}</button>
              </div>
            </div>
          </div>
        </div>
