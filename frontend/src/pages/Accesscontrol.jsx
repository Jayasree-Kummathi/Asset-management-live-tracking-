...
const downloadFile = async (url, filename) => {
  ...
  } catch (error) { console.error('Download error:', error); throw error; }
};
    ...
    <div className="ac-toast-container">
      {toasts.map(t => (
        <div key={t.id} className={`ac-toast ac-toast-${t.type}`}>
          ...
          <button className="ac-toast-close" onClick={() => setToasts(p => p.filter(x => x.id !== t.id))}><X size={12}/></button>
        </div>
      ))}
    </div>
  );
...
const getTypeIcon  = (type) => { const f = ACCESS_TYPES.find(t => t.value === type); if (!f) return <Shield size={12}/>; const I = f.icon; return <I size={12}/>; };
const getTypeLabel = (type) => { const f = ACCESS_TYPES.find(t => t.value === type); return f ? f.label : type; };
...
const pageStyles = `
  ...
  .ac-info-label { font-size: 11.5px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: .06em; }
  .ac-info-value { font-size: 13px; font-weight: 500; color: var(--text); text-align: right; }
`;

function Breadcrumb({ onBack, current }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:20, fontSize:13, color:'var(--text-muted)' }}>
      <button onClick={onBack} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:13, padding:0 }}>
        <ArrowLeft size={15}/> Access Control
      </button>
...
// ACCESS DETAIL PAGE
// ═══════════════════════════════════════════════════════════════════════════════
function AccessDetailPage({ record, onBack, onEdit, onRevoke, onSendReminder, sending }) {
  ...
  const isCritical = record.status === 'active' && days !== null && days >= 0 && days <= 2;
  const isActive   = !isExpired && !isRevoked;

  const statusColor = isRevoked ? 'var(--red)' : isExpired ? '#f59e0b' : isCritical ? 'var(--red)' : 'var(--green)';
  const statusLabel = isRevoked ? 'Revoked' : isExpired ? 'Expired' : isCritical ? 'Critical' : 'Active';

  return (
    <div className="fade-in" style={{ animation: 'slideInAC .22s cubic-bezier(.22,1,.36,1) both' }}>
      <style>{pageStyles}</style>
      <Breadcrumb onBack={onBack} current={`${record.emp_name} — Access Detail`} />

      {/* Header */}
      <div className="ac-detail-card" style={{ marginBottom: 20 }}>
        <div style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            ...
            <div>
              <h2 style={{ margin:0, fontSize:19, fontWeight:800, color:'var(--text)' }}>{record.emp_name}</h2>
              <div style={{ fontSize:13, color:'var(--text-muted)', marginTop:3 }}>
                {record.emp_id}{record.department ? ` · ${record.department}` : ''}{record.emp_email ? ` · ${record.emp_email}` : ''}
              </div>
            </div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
            <span className={`ac-status-badge ac-status-${record.status}`} style={{ fontSize:13, padding:'6px 14px' }}>
              {isRevoked ? <Lock size={13}/> : isExpired ? <AlertTriangle size={13}/> : <CheckCircle size={13}/>}
              &nbsp;{statusLabel}
            </span>
            {isActive && (
              <>
                {record.expiry_date && (
                  <button className="ac-btn ac-btn-ghost" onClick={() => onSendReminder(record.id, record.emp_name)}
                    disabled={sending === record.id}>
                    {sending === record.id ? <RefreshCw size={13} className="ac-spin"/> : <Mail size={13}/>}
                    &nbsp;Send Reminder
                  </button>
                )}
                <button className="ac-btn ac-btn-secondary" onClick={() => onEdit(record)}>
                  <Edit2 size={13}/> Edit
                </button>
                <button className="ac-btn ac-btn-danger" onClick={() => onRevoke(record)}>
                  <Lock size={13}/> Revoke
                </button>
              </>
            )}
            {(isRevoked || isExpired) && (
              <button className="ac-btn ac-btn-primary" onClick={() => onEdit(record)}>
                <Unlock size={13}/> Re-grant Access
              </button>
            )}
          </div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        ...
        <div>
          <div className="ac-detail-card">
            <div className="ac-detail-section-title"><Shield size={13}/> Access Details</div>
            <div className="ac-info-row">
              <span className="ac-info-label">Access Type</span>
              <span className="ac-info-value">
                <span className={`ac-type-tag ac-type-${record.access_type}`} style={{ fontSize:12 }}>
                  {getTypeIcon(record.access_type)}&nbsp;{getTypeLabel(record.access_type)}
                </span>
              </span>
            </div>
            {record.access_type === 'custom' && (
              <div className="ac-info-row">
                <span className="ac-info-label">Resource / URL</span>
                <span className="ac-info-value" style={{ fontFamily:'var(--mono)', fontSize:12, color:'var(--accent)' }}>
                  {record.access_value || '—'}
                </span>
              </div>
            )}
            <div className="ac-info-row">
              ...
              <span className="ac-info-value" style={{ color: statusColor, fontWeight:700 }}>{statusLabel}</span>
            </div>
            <div className="ac-info-row">
              <span className="ac-info-label">Granted On</span>
              <span className="ac-info-value" style={{ fontFamily:'var(--mono)', fontSize:12 }}>{formatDate(record.created_at)}</span>
            </div>
            <div className="ac-info-row">
              <span className="ac-info-label">Expiry Date</span>
              <span className="ac-info-value">
            ...
            {record.duration_days && (
              <div className="ac-info-row">
                <span className="ac-info-label">Duration</span>
                <span className="ac-info-value">{record.duration_days} days</span>
              </div>
            )}
          </div>
          ...
          {record.notes && (
            <div className="ac-detail-card">
              <div className="ac-detail-section-title">Notes</div>
              <div style={{
        ...
        {/* Right — employee info + expiry visual */}
        <div>
          <div className="ac-detail-card">
            <div className="ac-detail-section-title"><User size={13}/> Employee Info</div>
            <div className="ac-info-row">
              <span className="ac-info-label">Employee ID</span>
              <span className="ac-info-value" style={{ fontFamily:'var(--mono)', color:'var(--accent)', fontWeight:700 }}>{record.emp_id}</span>
            </div>
            <div className="ac-info-row">
              <span className="ac-info-label">Full Name</span>
              <span className="ac-info-value">{record.emp_name}</span>
            </div>
            <div className="ac-info-row">
              <span className="ac-info-label">Email</span>
              <span className="ac-info-value" style={{ fontSize:12 }}>{record.emp_email || '—'}</span>
            </div>
            <div className="ac-info-row">
              <span className="ac-info-label">Department</span>
              <span className="ac-info-value">{record.department || '—'}</span>
            </div>
          </div>

          {/* Expiry visual */}
          {record.expiry_date && (
            <div className="ac-detail-card" style={{
              background: isCritical ? 'rgba(239,68,68,.06)' : isExpired ? 'rgba(245,158,11,.06)' : 'rgba(52,211,153,.06)',
              border: `1px solid ${isCritical ? 'rgba(239,68,68,.25)' : isExpired ? 'rgba(245,158,11,.25)' : 'rgba(52,211,153,.25)'}`,
            }}>
              <div style={{ textAlign:'center', padding:'8px 0' }}>
                <div style={{ fontSize:36, fontWeight:900, color: statusColor, lineHeight:1 }}>
                  {days === null ? '∞' : days < 0 ? Math.abs(days) : days}
                </div>
                <div style={{ fontSize:13, color:'var(--text-muted)', marginTop:6 }}>
                  {days === null ? 'No expiry set' : days < 0 ? 'days overdue' : days === 0 ? 'expires today' : 'days remaining'}
                </div>
      ...
      <div style={{ marginTop:24, paddingTop:20, borderTop:'1px solid var(--border)' }}>
        <button className="ac-btn ac-btn-secondary" onClick={onBack}>
          <ArrowLeft size={14}/> Back to Access Control
        </button>
      </div>
    </div>
  );
...
// GRANT / EDIT PAGE
// ═══════════════════════════════════════════════════════════════════════════════
function GrantPage({ editing, onBack, onSaved }) {
  const isNew = !editing;
  ...
  return (
    <div className="fade-in" style={{ animation: 'slideInAC .22s cubic-bezier(.22,1,.36,1) both' }}>
      <style>{pageStyles}</style>
      <Breadcrumb onBack={onBack} current={isNew ? 'Grant New Access' : `Edit Access — ${editing.emp_name}`} />
        ...
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div>
            <h2 style={{ margin:0, fontSize:18, fontWeight:800, color:'var(--text)' }}>
              {isNew ? 'Grant New Access' : `Edit Access — ${editing.emp_name}`}
            </h2>
            <div style={{ fontSize:13, color:'var(--text-muted)', marginTop:3 }}>
              {isNew ? 'Fill in the details to grant system access' : 'Update access settings below'}
            </div>
          </div>
        </div>
      </div>

      <div className="ac-detail-card" style={{ padding:'24px 28px' }}>
        <form onSubmit={handleSubmit}>
          ...
          {/* Access Type */}
          <div className="ac-section-label" style={{ marginTop:20 }}>Access Type</div>
          <div className="ac-type-grid">
            {ACCESS_TYPES.map(t => (
              <button key={t.value} type="button"
                className={`ac-type-btn${form.accessType === t.value ? ' ac-type-active' : ''}`}
                onClick={() => set('accessType', t.value)} title={t.description}>
                <t.icon size={15}/>
          ...
          {form.accessType === 'custom' && (
            <div className="ac-form-group" style={{ marginTop:12 }}>
              ...
              <input required value={form.accessValue} onChange={e => set('accessValue', e.target.value)}
          ...
          {/* Duration */}
          <div className="ac-section-label" style={{ marginTop:20 }}>Duration</div>
          ...
          {!useCustom ? (
            <div className="ac-preset-grid">
              ...
              <input type="number" min="1" max="365" className="ac-preset-custom"
          ...
          <div style={{ display:'flex', gap:10, justifyContent:'flex-end', paddingTop:20, borderTop:'1px solid var(--border)', marginTop:8 }}>
            <button type="button" className="ac-btn ac-btn-secondary" onClick={onBack}>Cancel</button>
            <button type="submit" className="ac-btn ac-btn-primary" disabled={saving}>
              <Shield size={14}/>
              {saving ? 'Saving…' : isNew ? 'Grant Access' : 'Update Access'}
            </button>
          </div>
        </form>
      </div>
...
function AuditPage({ onBack }) {
  ...
  const fetchLogs = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ category:'access_control', limit:LIMIT, page, ...(search ? { search } : {}) });
      const data = await apiFetch(`/audit?${params}`);
      setLogs(data.data || []); setTotal(data.total || 0);
  ...
  const filtered     = actionFilt === 'all' ? logs : logs.filter(l => l.action === actionFilt);
  const actionTypes  = [...new Set(logs.map(l => l.action))];

  return (
    <div className="fade-in" style={{ animation: 'slideInAC .22s cubic-bezier(.22,1,.36,1) both' }}>
      <style>{pageStyles}</style>
      <Breadcrumb onBack={onBack} current="Audit Log" />
      ...
      <div className="ac-detail-card">
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div>
              <h3 style={{ margin:0, fontSize:16, fontWeight:700 }}>Access Control Audit Trail</h3>
              <p style={{ margin:0, fontSize:12, color:'var(--text-muted)' }}>{total} events recorded</p>
            </div>
          </div>
          <div className="ac-search" style={{ width:250 }}>
            <Search size={14}/>
        ...
        {/* Action filters */}
        <div className="ac-audit-filters" style={{ marginBottom:20 }}>
          <button className={`ac-audit-type-btn${actionFilt === 'all' ? ' active' : ''}`} onClick={() => setActionFilt('all')}>All</button>
          {actionTypes.map(a => {
            const m = auditMeta(a);
            return (
              <button key={a} className={`ac-audit-type-btn ac-audit-type-${m.color}${actionFilt === a ? ' active' : ''}`}
                onClick={() => setActionFilt(a)}>{m.icon} {m.label}</button>
            );
          })}
        </div>

        {/* Log list */}
        <div className="ac-audit-list" style={{ maxHeight: 'calc(100vh - 350px)', overflowY: 'auto' }}>
          {loading ? (
            <div className="ac-audit-loading">
              {[...Array(6)].map((_, i) => <div key={i} className="ac-audit-skeleton" style={{ animationDelay:`${i*60}ms` }}/>)}
            </div>
          ) : filtered.length === 0 ? (
            <div className="ac-audit-empty"><Activity size={32}/><p>No audit events found</p></div>
          ) : (
            filtered.map((log, idx) => {
              const meta = auditMeta(log.action);
              return (
                <div key={log.id} className={`ac-audit-item ac-audit-item-${meta.color}`} style={{ animationDelay:`${idx*20}ms` }}>
                  <div className={`ac-audit-dot ac-audit-dot-${meta.color}`}>{meta.icon}</div>
                  <div className="ac-audit-content">
                    <div className="ac-audit-row1">
                      <span className={`ac-audit-badge ac-audit-badge-${meta.color}`}>{meta.label}</span>
                    ...
                    <div className="ac-audit-meta">
                      ...
                      <span className="ac-audit-ts">{new Date(log.created_at).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              );
        ...
        {/* Pagination */}
        {total > LIMIT && (
          <div className="ac-audit-pagination" style={{ marginTop:20 }}>
            <button className="ac-btn ac-btn-secondary" disabled={page === 1} onClick={() => setPage(p => p-1)}>← Prev</button>
            <span>Page {page} of {Math.ceil(total / LIMIT)}</span>
            <button className="ac-btn ac-btn-secondary" disabled={page >= Math.ceil(total/LIMIT)} onClick={() => setPage(p => p+1)}>Next →</button>
          </div>
        )}
...
function ExportPage({ onBack }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (dateRange.start && dateRange.end) url += `?start=${dateRange.start}&end=${dateRange.end}`;
      const filename = `access-control-report-${new Date().toISOString().split('T')[0]}.xlsx`;
      await downloadFile(url, filename);
      toast.success('Report downloaded', filename);
    } catch (err) { toast.error('Failed to generate report', err.message); }
  ...
  const handleQuickExport = async (exportFormat) => {
    ...
    try {
      const url = exportFormat === 'excel'
        ? `${API}/access-control/report/excel`
        : `${API}/access-control/export/csv`;
      const filename = `access-control-${exportFormat}-${new Date().toISOString().split('T')[0]}.${exportFormat === 'excel' ? 'xlsx' : 'csv'}`;
      await downloadFile(url, filename);
      toast.success(`${exportFormat.toUpperCase()} exported`, filename);
  ...
  return (
    <div className="fade-in" style={{ animation: 'slideInAC .22s cubic-bezier(.22,1,.36,1) both' }}>
      <style>{pageStyles}</style>
      <Breadcrumb onBack={onBack} current="Export Access Control" />

      <div className="ac-detail-card">
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:24 }}>
          <Download size={20} style={{ color:'var(--accent)' }}/>
          <div>
            <h3 style={{ margin:0, fontSize:16, fontWeight:700 }}>Export Access Control Data</h3>
            <p style={{ margin:0, fontSize:12, color:'var(--text-muted)' }}>Download reports in Excel or CSV format</p>
          </div>
        </div>

        {/* Quick Export Buttons */}
        <div className="ac-section-label">Quick Export</div>
        <div className="ac-format-options" style={{ marginBottom:24 }}>
          <button className={`ac-format-btn ${format === 'excel' ? 'active' : ''}`} onClick={() => handleQuickExport('excel')} disabled={generating}>
            <FileSpreadsheet size={18}/> Excel (.xlsx)<small>Complete report with multiple sheets</small>
          </button>
          <button className={`ac-format-btn ${format === 'csv' ? 'active' : ''}`} onClick={() => handleQuickExport('csv')} disabled={generating}>
            <FileText size={18}/> CSV (.csv)<small>Simple single-sheet format</small>
          </button>
        </div>
        ...
        {/* Advanced Options */}
        <div className="ac-section-label">Advanced Export (with date filter)</div>
        <div className="ac-form-grid" style={{ gridTemplateColumns:'1fr 1fr', marginBottom:16 }}>
          <div className="ac-form-group">
          ...
          <div className="ac-form-group">
            ...
            <input type="date" value={dateRange.end}
              ...
              min={dateRange.start} max={new Date().toISOString().split('T')[0]}/>
          </div>
        </div>

        <div className="ac-info-box" style={{ marginBottom:20 }}>
          <Shield size={14}/>
          <span>Includes: Access records, employee details, expiry status, department breakdown, and statistics</span>
        </div>

        <div style={{ display:'flex', gap:10, justifyContent:'flex-end', paddingTop:20, borderTop:'1px solid var(--border)' }}>
          <button className="ac-btn ac-btn-secondary" onClick={onBack}>Cancel</button>
          <button className="ac-btn ac-btn-primary" onClick={handleGenerate} disabled={generating}>
            <Download size={14}/> {generating ? 'Generating...' : 'Generate Filtered Report'}
          </button>
        </div>
      </div>
    </div>
...
// ── Revoke Modal (small confirmation modal) ────────────────────────────────────
function RevokeModal({ record, onClose, onRevoked }) {
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      onRevoked(); onClose();
    } catch (err) { toast.error('Revoke failed', err.message); }
    finally { setLoading(false); }
  };
  return (
    ...
    <div className="ac-notif-wrap" ref={ref}>
      ...
      {open && (
        <div className="ac-notif-dropdown">
          <div className="ac-notif-header"><Bell size={13}/> Expiring Soon <span className="ac-notif-count">{alerts.length}</span></div>
          {alerts.length === 0 ? (
            <div className="ac-notif-empty"><CheckCircle size={22}/><p>All access is healthy</p></div>
          ) : (
              ...
              {alerts.map(r => {
                ...
                return (
                  <div key={r.id} className={`ac-notif-item${d <= 2 ? ' ac-notif-urgent' : ''}`}>
                    ...
                    <div className={`ac-notif-days${d <= 2 ? ' urgent' : ''}`}>{d === 0 ? 'Today' : `${d}d`}</div>
                  </div>
                );
              })}
            </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// MAIN PAGE (with simplified table)
// ═══════════════════════════════════════════════════════════════════════════════
export default function AccessControl() {
  ...
  const [showGrantPage, setShowGrantPage] = useState(false);
  ...
  const sendReminderEmail = async (id, empName) => {
    ...
    } catch (err) { toast.error('Reminder failed', err.message); }
    finally { setSending(null); }
  };

  // Navigation helpers
  const openView = (record) => {
    listStateSnapshot.current = { search, filter, typeFilter };
    setViewRecord(record);
    window.scrollTo({ top:0, behavior:'smooth' });
  };

  const openGrant = (record = null) => {
    listStateSnapshot.current = { search, filter, typeFilter };
    setEditRecord(record);
    setShowGrantPage(true);
    window.scrollTo({ top:0, behavior:'smooth' });
  };

  const openAudit = () => {
    listStateSnapshot.current = { search, filter, typeFilter };
    setShowAuditPage(true);
    window.scrollTo({ top:0, behavior:'smooth' });
  };
  ...
  const backToList = () => {
    ...
    setFilter(snap.filter || 'all');
    setTypeFilter(snap.typeFilter || 'all');
    setViewRecord(null);
    setShowGrantPage(false);
    setShowAuditPage(false);
    setShowExportPage(false);
    setEditRecord(null);
  };

  const handleSaved = async () => { await fetchRecords(); };
  const handleRevoked = async () => { await fetchRecords(); backToList(); };

  // Render inline pages
  if (viewRecord) {
    const fresh = records.find(r => r.id === viewRecord.id) || viewRecord;
    return (
      <>
        <ToastContainer/>
        <AccessDetailPage
          record={fresh}
          onBack={backToList}
          onEdit={(r) => { setViewRecord(null); openGrant(r); }}
  ...
  return (
    <div className="ac-page fade-in">
      ...
      <div className="ac-page-header">
        ...
        {/* Desktop actions */}
        <div className="ac-header-actions desktop-only">
          <NotificationBell records={records}/>
          <button className="ac-btn ac-btn-ghost" onClick={openAudit}>
            <Activity size={15}/> Audit Log
          </button>
          <button className="ac-btn ac-btn-secondary" onClick={openExport}>
            <Download size={15}/> Export
          </button>
          <button className="ac-btn ac-btn-primary" onClick={() => openGrant(null)}>
      ...
      {mobileMenuOpen && (
        <div className="ac-mobile-menu">
          <button onClick={() => { openAudit(); setMobileMenuOpen(false); }}>
            <Activity size={16}/> Audit Log
          </button>
          <button onClick={() => { openExport(); setMobileMenuOpen(false); }}>
            <Download size={16}/> Export
          </button>
          <button className="ac-btn-primary-mobile" onClick={() => { openGrant(null); setMobileMenuOpen(false); }}>
      ...
      <div className="ac-table-wrap">
        {loading ? (
          <div className="ac-loading">
            {[...Array(4)].map((_, i) => <div key={i} className="ac-skeleton" style={{ animationDelay:`${i*80}ms` }}/>)}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="ac-card-list">
            {filtered.map(r => {
              ...
              return (
                <div key={r.id} className="ac-record-card" onClick={() => openView(r)}>
                  <div className="ac-card-header">
                    <div className="ac-card-avatar">{r.emp_name?.[0] || '?'}</div>
                    <div className="ac-card-info">
                      <div className="ac-card-name">{r.emp_name}</div>
                      <div className="ac-card-meta">{r.emp_id} • {r.emp_email}</div>
                    </div>
                  ...
                  <div className="ac-card-details">
                    <div className="ac-card-type">
                      <span className={`ac-type-tag ac-type-${r.access_type}`}>
                        {getTypeIcon(r.access_type)}{getTypeLabel(r.access_type)}
                      </span>
                      {r.access_type === 'custom' && r.access_value && (
                        <span className="ac-card-url">{r.access_value}</span>
                      )}
                    </div>
                    <div className="ac-card-expiry">
                      <Clock size={12}/>
                      {r.expiry_date ? formatDate(r.expiry_date) : 'No expiry'}
                      <StatusChip days={days}/>
                    </div>
                  </div>
                  
                  <div className="ac-card-actions" onClick={e => e.stopPropagation()}>
                    {isActive && r.expiry_date && (
                      <button className="ac-action-btn" title="Send Reminder"
                        onClick={() => sendReminderEmail(r.id, r.emp_name)}
                        disabled={sending === r.id}>
                        {sending === r.id ? <RefreshCw size={14} className="ac-spin"/> : <Mail size={14}/>}
                      </button>
                    )}
                    <button className="ac-action-btn" title="Edit" onClick={() => openGrant(r)}>
                      <Edit2 size={14}/>
                    </button>
                    {isActive && (
                      <button className="ac-action-btn ac-action-danger" title="Revoke" onClick={() => setRevoking(r)}>
                        <Lock size={14}/>
                      </button>
                    )}
                    {(isRevoked || isExpired) && (
                      <button className="ac-action-btn ac-action-success" title="Re-grant" onClick={() => openGrant(r)}>
                        <Unlock size={14}/>
                      </button>
                    )}
                    <button className="ac-action-btn" title="View Details" onClick={() => openView(r)}>
                      <Eye size={14}/>
                    </button>
                  </div>
                </div>
      ...
      {revoking && (
        <RevokeModal record={revoking} onClose={() => setRevoking(null)} onRevoked={fetchRecords}/>
      )}
    </div>
  );
}