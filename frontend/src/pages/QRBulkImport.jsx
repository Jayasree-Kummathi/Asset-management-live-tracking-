...
import {
  Upload, FileText, Printer, CheckCircle, X,
  AlertTriangle, Package, Download, RefreshCw, Search
} from 'lucide-react';
...
const printQRLabels = (assets) => {
  ...
  import('react-dom/client').then(({ createRoot }) => {
    const roots = wrappers.map(({ asset, wrapper }) => {
      ...
      root.render(React.createElement(QRCodeCanvas, { value: buildQRData(asset), size: 150, level: 'M', includeMargin: false }));
      return { asset, wrapper };
    });
...
<html><head><title>QR Labels — Asset Management</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Courier New', monospace; padding: 10px; background: #fff; }
  .grid { display: flex; flex-wrap: wrap; gap: 8px; }
  .label { display: flex; flex-direction: column; align-items: center; padding: 8px 6px;
    border: 1px solid #aaa; border-radius: 6px; page-break-inside: avoid;
    background: #fff; width: 168px; gap: 3px; }
  ...
  @media print { body { padding: 4px; } @page { margin: 8mm; size: A4 portrait; } .grid { gap: 6px; } }
</style></head>
...
function ReprintQR({ allAssets }) {
  ...
  const toQRAsset      = (a) => ({ asset_id: a.id, serial: a.serial, brand: a.brand, model: a.model, config: a.config, processor: a.processor, ram: a.ram, storage: a.storage, vendor: a.vendor, location: a.location });

  return (
    <div>
      {/* Info banner */}
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', marginBottom:18, background:'var(--accent-glow, rgba(99,102,241,0.08))', border:'1px solid var(--accent)', borderRadius:'var(--radius)', fontSize:13, color:'var(--text-muted)' }}>
        <RefreshCw size={16} style={{ color:'var(--accent)', flexShrink:0 }}/>
        <span>QR sticker lost or damaged? Search for the laptop below, select it, and reprint its QR label. Works for <strong style={{ color:'var(--text)' }}>any status</strong> — Stock, Allocated, Repair, etc.</span>
      </div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap' }}>
        <div className="search-bar" style={{ flex:1, minWidth:200 }}>
          <Search size={14}/>
          <input className="form-input" placeholder="Search asset ID, serial, brand, model…" value={search} onChange={e => setSearch(e.target.value)}/>
        </div>
        <div className="toggle-group">
          {STATUS_OPTIONS.map(s => (
            <button key={s} className={`toggle-btn ${statusFilter === s ? 'active' : ''}`} onClick={() => setStatusFilter(s)} style={{ fontSize:12, padding:'4px 10px' }}>{s}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxHeight:340, overflowY:'auto', marginBottom:14 }} className="table-wrap">
        <table>
          <thead>
            <tr>
              <th style={{ width:36 }}>
                <input type="checkbox" checked={filtered.length > 0 && selected.size === filtered.length} onChange={toggleAll} style={{ accentColor:'var(--accent)' }}/>
              </th>
              <th>Asset ID</th><th>Serial</th><th>Brand / Model</th><th>Config</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6}><div className="empty-state"><p>No assets found</p></div></td></tr>
            ) : filtered.map(a => (
              <tr key={a.id} style={{ opacity: selected.has(a.id) ? 1 : 0.6 }}>
                <td><input type="checkbox" checked={selected.has(a.id)} onChange={() => toggleSelect(a.id)} style={{ accentColor:'var(--accent)' }}/></td>
                ...
                <td>
                  <span style={{ fontSize:11, padding:'2px 8px', borderRadius:20, fontWeight:600, background: a.status === 'Stock' ? 'var(--green-bg)' : a.status === 'Allocated' ? 'var(--accent-glow)' : 'var(--surface2)', color: a.status === 'Stock' ? 'var(--green)' : a.status === 'Allocated' ? 'var(--accent)' : 'var(--text-muted)' }}>{a.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {selected.size > 0 && (
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:13, fontWeight:600, marginBottom:10, color:'var(--text)' }}>QR Preview — {selected.size} selected</div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:10, padding:14, background:'var(--surface2)', borderRadius:'var(--radius)', maxHeight:280, overflowY:'auto' }}>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', gap:10 }}>
        ...
        <button className="btn btn-primary" disabled={selected.size === 0} onClick={() => printQRLabels(selectedAssets.map(toQRAsset))}>
...
// ══════════════════════════════════════════════════════════════════════════════
// ── MAIN COMPONENT — now renders INLINE (no modal-overlay wrapper) ─────────────
// ══════════════════════════════════════════════════════════════════════════════
...
LTB-302,DX202,HP,ProBook 450,i5/8GB/256GB,Intel i5-1335U,8GB DDR5,256GB SSD,2024-02-01,2024-02-01,2027-02-01,HP India,Mumbai`;
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setAssets(parsed); setSelected(new Set(parsed.map(a => a.asset_id))); setStep('preview');
  ...
  const toggleSelect = (id) => setSelected(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  const toggleAll    = () => setSelected(prev => prev.size === assets.length ? new Set() : new Set(assets.map(a => a.asset_id)));

  // ── Inline title (used by Inventory.js PageHeader — we just render content) ──
  const tabTitle = activeTab === 'reprint'
    ? 'Reprint QR Labels'
    : step === 'preview' ? `Preview — ${assets.length} Laptops`
    : step === 'qr'      ? 'QR Labels Ready to Print'
    : 'Import Laptops & Generate QR Codes';

  return (
    /* ── ROOT: plain div, NO modal-overlay, NO backdrop ── */
    <div className="fade-in" style={{ animation:'slideInQR 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      <style>{`@keyframes slideInQR{from{opacity:0;transform:translateX(24px)}to{opacity:1;transform:translateX(0)}}`}</style>
      ...
      <div className="card" style={{ marginBottom:0, padding:'0', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px', borderBottom:'1px solid var(--border)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <h2 style={{ fontSize:17, fontWeight:700, margin:0 }}>{tabTitle}</h2>
          </div>
        </div>
        ...
        <div style={{ display:'flex', gap:0, borderBottom:'1px solid var(--border)', padding:'0 24px', background:'var(--surface)' }}>
          {[
            ...
            <button key={tab.key}
              onClick={() => { setActiveTab(tab.key); setStep('import'); setError(''); }}
              style={{
        ...
        <div style={{ padding:'24px' }}>
          ...
          {/* ── REPRINT TAB ── */}
          {activeTab === 'reprint' && <ReprintQR allAssets={allAssets}/>}
          ...
          {activeTab === 'import' && (
            <>
              ...
              {step === 'import' && (
                <div>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', marginBottom:20, background:'var(--accent-glow, rgba(99,102,241,0.08))', border:'1px solid var(--accent)', borderRadius:'var(--radius)' }}>
                    <div>
                      <div style={{ fontWeight:600, fontSize:13 }}>📥 Step 1 — Download the Excel Template</div>
                      <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:3 }}>Fill in asset details and upload below</div>
                    </div>
                  ...
                  <div
                    style={{ border:'2px dashed var(--border)', borderRadius:'var(--radius)', padding:22, textAlign:'center', marginBottom:20, cursor:'pointer', background:'var(--surface2)' }}
                  ...
                  {error && (
                    <div style={{ color:'var(--red)', fontSize:13, marginBottom:12, padding:'10px 14px', background:'var(--red-bg)', borderRadius:'var(--radius)' }}>
                ...
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      <button className="btn btn-secondary btn-sm" onClick={toggleAll}>{selected.size === assets.length ? 'Deselect All' : 'Select All'}</button>
                  ...
                  <div style={{ maxHeight:380, overflowY:'auto', marginBottom:16 }} className="table-wrap">
                    <table>
                      <thead>
                        <tr>
                          <th style={{ width:36 }}><input type="checkbox" checked={selected.size === assets.length} onChange={toggleAll} style={{ accentColor:'var(--accent)' }}/></th>
                      ...
                      <tbody>
                        {assets.map(a => (
                          <tr key={a.asset_id} style={{ opacity: selected.has(a.asset_id) ? 1 : 0.45 }}>
                            <td><input type="checkbox" checked={selected.has(a.asset_id)} onChange={() => toggleSelect(a.asset_id)} style={{ accentColor:'var(--accent)' }}/></td>
                ...
                <div>
                  {result && (
                    <div style={{ display:'flex', gap:12, marginBottom:20 }}>
                      <div style={{ flex:1, padding:'12px 16px', background:'var(--green-bg)', border:'1px solid rgba(52,211,153,.2)', borderRadius:'var(--radius)', textAlign:'center' }}>
                      ...
                      {result.failed.length > 0 && (
                        <div style={{ flex:1, padding:'12px 16px', background:'var(--red-bg)', border:'1px solid rgba(248,113,113,.2)', borderRadius:'var(--radius)', textAlign:'center' }}>
                  ...
                  {result?.failed?.length > 0 && (
                    <div style={{ marginBottom:14 }}>
                      {result.failed.map((f, i) => <div key={i} style={{ fontSize:12, color:'var(--red)', marginBottom:3 }}>{f.id}: {f.reason}</div>)}
                    </div>
                  )}
                    ...
                    <>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
                        <div style={{ fontSize:13, fontWeight:600 }}>
                          ...
                          <span style={{ fontSize:12, fontWeight:400, color:'var(--text-muted)', marginLeft:8 }}>Print and stick on laptops</span>
                        </div>
                      ...
                      <div style={{ display:'flex', flexWrap:'wrap', gap:12, padding:16, background:'var(--surface2)', borderRadius:'var(--radius)', maxHeight:460, overflowY:'auto', marginBottom:16 }}>
