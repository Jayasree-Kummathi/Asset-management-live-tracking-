...
const EMPTY_EDIT = {
  ...
  notes: '', cc_emails: '', portal_url: '', photo_url: '',
};
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8 }}>
        <input
          type="text" className="form-input" style={{ flex:1, fontSize:12 }}
          value={ccInput}
          onChange={e => { setCcInput(e.target.value); setCcError(''); }}
          onKeyDown={handleKey}
          placeholder="Type email and press Enter or comma…"
        />
        <button type="button" className="btn btn-secondary btn-sm" onClick={addTag} disabled={!ccInput.trim()}>
          <Plus size={13}/> Add
        </button>
      </div>
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:14 }}>
      <div
        ...
        style={{ width:64, height:64, borderRadius:'50%', overflow:'hidden', border:'2px dashed var(--border)', background:'var(--surface2)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0 }}
      >
        {preview
          ? <img src={preview} alt="emp" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        ...
        <button type="button" className="btn btn-secondary btn-sm" onClick={() => photoRef.current?.click()}>
          <Camera size={12}/> {preview ? 'Change' : 'Upload Photo'}
        </button>
        {preview && (
          <button type="button" className="btn btn-sm" style={{ fontSize:11 }} onClick={() => { onPhotoRemove(); if(photoRef.current) photoRef.current.value=''; }}>
            <X size={11}/> Remove
          </button>
        )}
      </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// INLINE EDIT PAGE  — now includes Photo upload + CC emails
// ══════════════════════════════════════════════════════════════════════════════
function InlineEditPage({ employees, editIdx, editedSet, onSave, onCancel, onNav }) {
  ...
  const setField = (k, v) => {
    setForm(f => ({ ...f, [k]: v }));
    setChangedFields(prev => new Set([...prev, k]));
  };

  const handleCcChange = (tags) => {
    setCcTags(tags);
    setField('cc_emails', tags.join(', '));
  };

  const handlePhotoChange = (dataUrl) => {
    setPreview(dataUrl);
    setField('photo_url', dataUrl);
  };
  ...
  const handleSave = () => onSave(editIdx, { ...form, cc_emails: ccTags.join(', '), photo_url: preview }, changedFields.size > 0);

  const handleNav = (dir) => {
    onSave(editIdx, { ...form, cc_emails: ccTags.join(', '), photo_url: preview }, changedFields.size > 0);
    onNav(dir);
  };

  const total    = employees.length;
  const isFirst  = editIdx === 0;
  const isLast   = editIdx === total - 1;
    ...
    <div style={{ animation: 'slideInRight 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'20px 28px', marginBottom:20, display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>
            {[form.emp_id, form.designation, form.location].filter(Boolean).join(' · ')}
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          <button className="btn btn-secondary btn-sm" disabled={isFirst} onClick={() => handleNav(-1)}>
            <ChevronLeft size={14}/> Prev
          </button>
          <span style={{ fontSize:12, color:'var(--text-muted)', minWidth:60, textAlign:'center' }}>
            {editIdx + 1} / {total}
          </span>
          <button className="btn btn-secondary btn-sm" disabled={isLast} onClick={() => handleNav(1)}>
      ...
      {hasChanges && (
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 14px', marginBottom:16, background:'rgba(59,184,122,0.08)', border:'1px solid rgba(59,184,122,0.25)', borderRadius:8, fontSize:13, color:'#1a8050' }}>
          <Check size={14}/>
          Unsaved changes — click "Save &amp; back to review" to apply
        </div>
      )}
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'24px 28px' }}>
        ...
        <div style={{ marginBottom:4 }}>
          ...
          <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:6 }}>
            Photo will appear in the welcome email and employee profile.
          </div>
        </div>
        ...
        <div className="form-grid form-grid-3">
          <FormField label="Employee ID *">
            <input
              ...
              placeholder="e.g. IBC001"
              style={changedFields.has('emp_id') ? { borderColor:'var(--accent)', background:'var(--accent-glow)' } : {}}
            />
            <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:3 }}>
              Edit to correct auto-generated IDs
            </div>
          </FormField>
          <FormField label="Full name *" span2>
            <input className="form-input" required value={form.emp_name}
              onChange={e => setField('emp_name', e.target.value)} placeholder="Jacob Thomas"/>
          </FormField>
          <FormField label="Date of joining">
            <input type="date" className="form-input" value={form.doj}
        ...
        <div className="form-grid form-grid-3">
          ...
          <FormField label="Designation" span2>
            <input className="form-input" value={form.designation}
              onChange={e => setField('designation', e.target.value)} placeholder="Software Engineer"/>
          </FormField>
          <FormField label="Service line">
            <input className="form-input" value={form.service_line}
              onChange={e => setField('service_line', e.target.value)} placeholder="Engineering"/>
          </FormField>
          <FormField label="Client">
            <input className="form-input" value={form.client}
              onChange={e => setField('client', e.target.value)} placeholder="Client name"/>
          </FormField>
          <FormField label="Location">
            <input className="form-input" value={form.location}
              onChange={e => setField('location', e.target.value)} placeholder="Bengaluru"/>
          </FormField>
          <FormField label="Reporting manager" span2>
            <input className="form-input" value={form.reporting_manager}
              onChange={e => setField('reporting_manager', e.target.value)} placeholder="Manager name"/>
          </FormField>
        ...
        <div className="form-grid form-grid-3">
          <FormField label="Mobile number">
            <input className="form-input" type="tel" value={form.mobile_no}
              onChange={e => setField('mobile_no', e.target.value)} placeholder="+91 98765 43210"/>
          </FormField>
          <FormField label="Company email" span2>
            <input type="email" className="form-input" value={form.company_email}
              onChange={e => setField('company_email', e.target.value)} placeholder="jacob@mindteck.com"/>
          </FormField>
          <FormField label="Suggested / alternative email" span2>
            <input type="email" className="form-input" value={form.suggested_email}
              onChange={e => setField('suggested_email', e.target.value)} placeholder="jacob.thomas@mindteck.com"/>
          </FormField>
          <FormField label="Personal email">
            <input type="email" className="form-input" value={form.personal_email}
              onChange={e => setField('personal_email', e.target.value)} placeholder="jacob@gmail.com"/>
          </FormField>
        </div>
        ...
        <div style={{ marginBottom:4 }}>
          <div style={{ fontSize:11, color:'var(--text-muted)', marginBottom:8, lineHeight:1.6 }}>
            Add manager / HR emails to CC on the welcome email. Employee's own emails are excluded automatically.
          </div>
        ...
        <div className="form-grid form-grid-2">
          <FormField label="Company email password">
            <input className="form-input" value={form.company_email_password}
              onChange={e => setField('company_email_password', e.target.value)}
              placeholder="e.g. Mindteck@2024"/>
          </FormField>
          <FormField label="Password hint">
            <input className="form-input" value={form.password_hint}
              onChange={e => setField('password_hint', e.target.value)}
              placeholder="e.g. First letter uppercase + @year"/>
          </FormField>
        </div>

        {/* ── Portal ── */}
        <BulkSectionHeader icon={Globe} title="Portal URL"/>
        <div className="form-group">
          <input type="url" className="form-input" value={form.portal_url}
            onChange={e => setField('portal_url', e.target.value)}
        ...
        <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:20, paddingTop:16, borderTop:'1px solid var(--border)' }}>
          ...
          <button className="btn btn-primary" onClick={handleSave}>
            <Check size={14}/> Save &amp; back to review
          </button>
        </div>
      </div>
    </div>
...
function BulkImportPage({ onBack, onImport }) {
  ...
  const FIELD_MAP = {
    ...
    'name':'emp_name','full name':'emp_name','doj':'doj','date of joining':'doj',
    'joining date':'doj','level':'level','designation':'designation','title':'designation',
    'role':'designation','location':'location','mobile number':'mobile_no','mobile no':'mobile_no',
    'mobile':'mobile_no','phone':'mobile_no','service line':'service_line','service':'service_line',
    ...
    'personal email':'personal_email','blood group':'blood_group','blood':'blood_group',
    'dob':'dob','date of birth':'dob','company email':'company_email','email':'company_email',
    ...
    'notes':'notes','cc emails':'cc_emails','cc_emails':'cc_emails',
  };
  ...
  // ── FIX: better auto emp_id using sequential counter + prefix ────────────
  ...
  const parseTabularData = () => {
    ...
    const mapped  = headers.map(h => FIELD_MAP[h.trim().toLowerCase()] || h.trim().toLowerCase());
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const first  = padded[0].trim();
      const isCont = rows.length > 0 && (
        !first ||
        BLOOD_RE.test(first) ||
        /^\d{1,2}[-/][A-Za-z]{3}[-/]\d{2,4}$/.test(first) ||
        /^\d{4}-\d{2}-\d{2}$/.test(first) ||
        /^\d{1,2}[-/]\d{1,2}[-/]\d{4}$/.test(first) ||
        (!EMP_ID_RE.test(first) && cells.filter(isMeaningful).length <= 3)
      );
      if (isCont) {
        const prev = rows[rows.length - 1];
        padded.forEach((val, idx) => { if (isMeaningful(val) && !isMeaningful(prev[idx])) prev[idx] = val.trim(); });
      } else {
        rows.push(padded.map(v => v.trim()));
      }
    }

    const employees = rows.map((cells, rowIndex) => {
      const emp = {};
      mapped.forEach((field, idx) => { const v = (cells[idx] || '').trim(); if (isMeaningful(v)) emp[field] = v; });

      // Auto-generate company email from name if missing
      ...
      // ── FIX: better emp_id generation ──
      if (!emp.emp_id && emp.emp_name) {
        emp.emp_id = generateEmpId(emp, rowIndex);
      }
    ...
    if (!employees.length) { setError('No valid employee data found.'); return; }
  ...
  const formatDateForAPI = (ds) => {
    ...
    try {
      if (/\d{1,2}-[A-Za-z]{3}-\d{2,4}/.test(ds)) {
        ...
        const mm = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'}[m.toLowerCase().slice(0,3)];
      ...
      const d = new Date(ds); return isNaN(d.getTime()) ? null : d.toISOString().split('T')[0];
    } catch { return null; }
  };
    ...
    for (const emp of parsedData) {
      try {
        ...
        await apiFetch('/employees', {
          method: 'POST',
          body: JSON.stringify({
            emp_id: emp.emp_id, emp_name: emp.emp_name, doj: emp.doj||null,
            level: emp.level||null, designation: emp.designation||null,
            location: emp.location||null, mobile_no: emp.mobile_no||null,
            service_line: emp.service_line||null, client: emp.client||null,
            ...
            blood_group: emp.blood_group||null, dob: emp.dob||null,
            ...
            photo_url: emp.photo_url||null, status: emp.status||'Active',
            notes: emp.notes||null, cc_emails: emp.cc_emails||null,
            portal_url: emp.portal_url||null,
          }),
        });
        ok++;
      } catch (e) { fail++; errs.push(`${emp.emp_name || emp.emp_id}: ${e.message}`); }
    }
    const edited = editedSet.current.size;
    let msg = `Import done!\n✅ Success: ${ok}\n❌ Failed: ${fail}`;
    if (edited) msg += `\n✏️ ${edited} record${edited !== 1 ? 's' : ''} manually edited before import`;
    if (sendEmails) msg += `\n📧 Welcome emails sent.`; else msg += `\n⏸️ No emails sent.`;
    if (errs.length) msg += `\n\nErrors:\n${errs.slice(0, 5).join('\n')}`;
    alert(msg);
    ...
    return (
      <div style={{ animation: 'fadeIn 0.18s ease both' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:20, fontSize:13, color:'var(--text-muted)' }}>
          ...
          <ChevronRight size={13} style={{ opacity:0.4 }}/>
          <span style={{ color:'var(--text)', fontWeight:600 }}>Bulk Import — Review</span>
        </div>
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'24px 28px', marginBottom:24 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18, flexWrap:'wrap', gap:12 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:4 }}>
                Click ✏️ Edit to add photo, CC emails, or correct any field
              </div>
            </div>
            ...
            <table style={{ fontSize:12, width:'100%' }}>
              ...
              <tbody>
                {parsedData.map((emp, idx) => (
                  <tr key={idx} style={{ borderBottom:'1px solid var(--border)', background: editedSet.current.has(idx) ? 'rgba(59,184,122,0.04)' : undefined }}>
                    <td style={{ padding:'8px 12px', textAlign:'center', color:'var(--text-muted)' }}>{idx + 1}</td>
                    <td style={{ padding:'8px 12px' }}>
                      <BulkAvatar photo={emp.photo_url} name={emp.emp_name} size={32}/>
                    </td>
                    <td style={{ padding:'8px 12px', fontFamily:'var(--mono)', fontSize:11, color:'var(--accent)', fontWeight:700 }}>{emp.emp_id}</td>
                    <td style={{ padding:'8px 12px' }}>
                      <span style={{ fontWeight:600 }}>{emp.emp_name}</span>
                      {editedSet.current.has(idx) && (
                        <span style={{ marginLeft:7, fontSize:10, fontWeight:700, padding:'1px 7px', borderRadius:20, background:'rgba(59,184,122,0.12)', color:'#1a8050', border:'1px solid rgba(59,184,122,0.25)' }}>
                          edited
                        </span>
                      )}
                    </td>
                    <td style={{ padding:'8px 12px', color:'var(--text-muted)' }}>{emp.designation || '—'}</td>
                    <td style={{ padding:'8px 12px' }}>
                      {emp.level
                    ...
                    <td style={{ padding:'8px 12px', color:'var(--text-muted)', fontSize:11 }}>{emp.company_email || '—'}</td>
                    <td style={{ padding:'8px 12px', fontSize:11 }}>
                      {emp.cc_emails
                        ? <span style={{ color:'var(--accent)', fontWeight:600 }}>✓ {emp.cc_emails.split(',').filter(Boolean).length}</span>
                        : <span style={{ color:'var(--text-muted)' }}>—</span>}
                    </td>
                    <td style={{ padding:'8px 12px', fontFamily:'var(--mono)', fontSize:11 }}>{emp.doj || '—'}</td>
                    <td style={{ padding:'8px 12px', color:'var(--text-muted)' }}>{emp.location || '—'}</td>
                    <td style={{ padding:'8px 12px' }}>
                      <button className="btn btn-sm btn-secondary" onClick={() => openEdit(idx)} title="Edit — add photo, CC, fix ID">
                        <Edit2 size={12}/> Edit
                      </button>
                    </td>
                  </tr>
          ...
          <div style={{ display:'flex', gap:10, justifyContent:'flex-end', marginTop:20 }}>
            ...
            <button className="btn btn-primary" onClick={handleImport} disabled={importing} style={{ minWidth:240 }}>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'24px 28px', marginBottom:24 }}>
        <div style={{ marginBottom:18, padding:'12px 16px', background:'#EFF6FF', borderRadius:8, border:'1px solid #BFDBFE', fontSize:12, color:'#1E3A8A', lineHeight:1.7 }}>
          ...
          <ul style={{ margin:'6px 0 0 18px' }}>
            ...
            <li>Company email and Employee ID are auto-generated if missing — Employee ID format: initials + 3-digit number (e.g. JT001)</li>
            <li>After parsing, click <strong>✏️ Edit</strong> on any row to add a photo or CC email recipients</li>
          </ul>
        </div>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            placeholder={'Emp ID\tEmployee Name\tDOJ\tLevel\tDesignation\tLocation\tMobile Number\tService Line\tClient\tReporting Manager\tSuggested Email Id\tPersonal Email\tBlood Group\tDOB'}
  ...
  return (
    <div className="fade-in" style={{ animation:'slideInExit 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ background:'linear-gradient(135deg,rgba(20,83,45,0.06),rgba(22,101,52,0.02))', border:'1px solid #86efac', borderRadius:16, padding:'20px 28px', marginBottom:24, display:'flex', alignItems:'center', gap:16 }}>
        <div style={{ width:48, height:48, borderRadius:12, background:'linear-gradient(135deg,#14532d,#166534)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 4px 12px rgba(20,83,45,0.3)' }}>
        ...
        <div style={{ fontSize:12, fontWeight:700, background:'#fef2f2', color:'#991b1b', padding:'4px 14px', borderRadius:20, border:'1px solid #fca5a5', flexShrink:0 }}>
          ⚠️ Permanent Delete
        </div>
      </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'16px 20px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color: allChecked ? '#166534' : 'var(--text-muted)', fontFamily:'var(--mono)' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'16px 20px' }}>
            <div style={{ fontSize:13, fontWeight:700, marginBottom:4, display:'flex', alignItems:'center', gap:6 }}>
              <Mail size={13} color="var(--accent)"/>CC — Exit Notification Recipients
            </div>
          ...
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'16px 20px' }}>
            <label className="form-label" style={{ marginBottom:8, display:'flex', alignItems:'center', gap:6 }}>
              📝 Admin Notes
              <span style={{ fontWeight:400, color:'var(--text-muted)', fontSize:11 }}>(optional — appears in exit email)</span>
            </label>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <button className="btn btn-danger" onClick={handleConfirm} disabled={deleting} style={{ width:'100%', justifyContent:'center', padding:'12px 20px', fontSize:14, fontWeight:700 }}>
              <Trash2 size={15}/>
              {deleting ? 'Deleting employee…' : `Confirm Exit & Delete ${emp.emp_name}`}
            </button>
...
function DeletedEmployeesPage({ onBack }) {
  ...
  const filtered = deleted.filter(e =>
    !search ||
    [e.emp_id, e.emp_name, e.company_email, e.designation, e.service_line, e.location]
    ...
    <div className="fade-in" style={{ animation:'slideInDeleted 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'28px 32px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            ...
            <div>
              ...
              <p style={{ fontSize:12, color:'var(--text-muted)', margin:'3px 0 0' }}>
                {deleted.length} record{deleted.length !== 1 ? 's' : ''} removed from the system
              </p>
            </div>
          </div>
        ...
        <div className="search-bar" style={{ marginBottom:16, maxWidth:420 }}>
          <Search size={14}/>
          <input className="form-input" placeholder="Search by name, ID, email, designation…" value={search} onChange={e => setSearch(e.target.value)}/>
        </div>

        {!loading && (
          <div style={{ marginBottom:12, fontSize:12, color:'var(--text-muted)' }}>
            Showing {filtered.length} of {deleted.length} records
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p style={{ color:'var(--text-muted)' }}>{deleted.length === 0 ? 'No deleted employees yet' : 'No results match your search'}</p>
          </div>
        ) : (
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th>Service Line</th><th>Location</th><th>Company Email</th><th>Deleted At</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((e, idx) => (
                  <tr key={e.emp_id} style={{ opacity: 0.78 }}>
                    ...
                    <td>
                      <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11.5, color:'#ef4444', fontFamily:'var(--mono)' }}>
                        <Clock size={11}/> {e.deleted_at_fmt || '—'}
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          {emp.portal_url && (
            <div>
              ...
              <div style={{ padding:'12px 16px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10 }}>
                <div style={{ fontSize:11, color:'var(--text-muted)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:6, display:'flex', alignItems:'center', gap:6 }}><Globe size={11}/> Portal URL</div>
...
function EmployeeEditPage({ emp, onBack, onSaved, isAdmin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiFetch(`/employees/${emp.emp_id}`, { method:'PUT', body: JSON.stringify({ ...form, cc_emails: ccTags.join(', ') }) });
      if (onSaved) onSaved();
      onBack();
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'28px 32px', marginBottom:24 }}>
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-grid form-grid-3">
            ...
            <div className="form-group"><label className="form-label">Date of Joining</label><input type="date" className="form-input" value={form.doj} onChange={e => setField('doj', e.target.value)}/></div>
            <div className="form-group"><label className="form-label">Date of Birth</label><input type="date" className="form-input" value={form.dob} onChange={e => setField('dob', e.target.value)}/></div>
          ...
          <div className="form-grid form-grid-2">
            <div className="form-group" style={{ gridColumn:'span 2' }}><label className="form-label">Portal URL</label><input type="url" className="form-input" value={form.portal_url} onChange={e => setField('portal_url', e.target.value)} placeholder="https://your-portal.com"/><div style={{ fontSize:11, color:'var(--text-muted)', marginTop:3 }}>This URL will appear in the welcome email.</div></div>
          </div>
          <Section icon={Lock} title="Login Credentials"/>
          <div className="form-grid form-grid-2">
            <div className="form-group"><label className="form-label"><Lock size={11} style={{ display:'inline', marginRight:4 }}/>Company Email Password</label><PasswordInput value={form.company_email_password} onChange={e => setField('company_email_password', e.target.value)} placeholder="Initial password (e.g. Mindteck@2024)"/></div>
            <div className="form-group"><label className="form-label"><Key size={11} style={{ display:'inline', marginRight:4 }}/>Password Hint / Notes</label><input className="form-input" value={form.password_hint} onChange={e => setField('password_hint', e.target.value)} placeholder="e.g. First letter uppercase + @year"/></div>
          </div>
...
function AddEmployeePage({ onBack, onSaved, isAdmin }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); setSaving(true);
    try {
      await apiFetch('/employees', { method:'POST', body: JSON.stringify({ ...form, cc_emails: ccTags.join(', ') }) });
      if (onSaved) onSaved();
      onBack();
    } catch (err) { alert(err.message); }
    finally { setSaving(false); }
    ...
    <div className="fade-in" style={{ animation:'slideInAdd 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'28px 32px', marginBottom:24 }}>
        <form onSubmit={handleSubmit}>
          ...
          <Section icon={User} title="Basic Information"/>
          <div className="form-grid form-grid-3">
            <div className="form-group"><label className="form-label">Employee ID *</label><input className="form-input" required value={form.emp_id} onChange={e => setField('emp_id', e.target.value)} placeholder="EMP-001"/></div>
...
function SupportContactsPage({ onBack, isAdmin }) {
  ...
  const [saving,         setSaving]         = useState(false);
  const [formData,       setFormData]       = useState({ name:'', phone:'', email:'', role:'IT Support', sort_order:0, is_active:true });
  ...
  const handleSave = async (e) => {
    e.preventDefault(); setSaving(true);
    try {
      if (editingContact) await apiFetch(`/employees/support-contacts/${editingContact.id}`, { method:'PUT', body: JSON.stringify(formData) });
      else                await apiFetch('/employees/support-contacts', { method:'POST', body: JSON.stringify(formData) });
  ...
  return (
    <div className="fade-in" style={{ animation:'slideInSupport 0.22s cubic-bezier(0.22,1,0.36,1) both' }}>
      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:'28px 32px', marginBottom:24 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
          <div>
            <h2 style={{ fontSize:18, fontWeight:600, display:'flex', alignItems:'center', gap:8 }}><HelpCircle size={20} color="var(--accent)"/>IT Support Contacts</h2>
        ...
        <div className="inv-toolbar" style={{ marginBottom:16 }}>
          <div className="search-bar" style={{ flex:1, maxWidth:360 }}>
            <Search size={15}/><input className="form-input" placeholder="Search by name, email, phone, role..." value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={isAdmin ? 7 : 6}><div className="empty-state"><Users size={32} style={{ opacity:0.3 }}/><p>No support contacts found</p>{isAdmin && <button className="btn btn-primary" onClick={() => setShowModal(true)}><Plus size={14}/> Add First Contact</button>}</div></td></tr>
              ) : filteredContacts.map((contact, idx) => (
                <tr key={contact.id}>
                  <td style={{ textAlign:'center', fontFamily:'monospace', fontSize:12, color:'var(--text-muted)' }}>{contact.sort_order !== undefined ? contact.sort_order + 1 : idx + 1}</td>
          ...
          <div className="modal" style={{ maxWidth:500 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <form onSubmit={handleSave}>
                ...
                <div className="form-group"><label className="form-label">Phone Number</label><input className="form-input" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} placeholder="e.g., 9916675460"/></div>
...
export default function Employees() {
  ...
  const handleEditSaved = async () => { await fetchEmployees(); };
  ...
  if (editEmp)         return <div className="fade-in"><EmployeeEditPage    emp={editEmp}   onBack={handleBackToList} onSaved={handleEditSaved} isAdmin={isAdmin}/></div>;
  if (showAddPage)     return <div className="fade-in"><AddEmployeePage     onBack={handleBackToList} onSaved={fetchEmployees} isAdmin={isAdmin}/></div>;
  ...
  return (
    <div className="fade-in">
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          {isAdmin && (
            <>
              ...
              <button className="btn btn-secondary" onClick={() => setShowDeletedPage(true)} style={{ color:'#ef4444', borderColor:'#fca5a5' }}>
      ...
      <div style={{ display:'flex', gap:0, marginBottom:20, borderBottom:'1px solid var(--border)' }}>
        <button className="btn" onClick={() => setActiveTab('employees')} style={{ padding:'10px 20px', border:'none', background:'none', cursor:'pointer', fontWeight:500, color: activeTab === 'employees' ? 'var(--accent)' : 'var(--text-muted)', borderBottom: activeTab === 'employees' ? '2px solid var(--accent)' : 'none', borderRadius:0 }}>
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>Company Email</th><th>Portal URL</th><th>Status</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
