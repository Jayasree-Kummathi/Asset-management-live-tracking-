...
function parseSafeDate(d) {
  ...
  if (m1) {
    const mon = MONTH_NAMES_MAP[m1[2].toLowerCase()];
    if (mon !== undefined) {
      let yr = parseInt(m1[3], 10);
      if (yr < 100) yr += yr > 50 ? 1900 : 2000;
      return new Date(yr, mon, parseInt(m1[1], 10));
    }
  }
  const m2 = s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (m2) {
    const dt = new Date(parseInt(m2[3]), parseInt(m2[2], 10) - 1, parseInt(m2[1], 10));
    return isNaN(dt) ? null : dt;
  }
...
// ── FALLBACK: regex-based parser (kept for offline/error cases) ───────────────
const HEADER_MAP = {
  empId: ['eid','emp id','employee id','empid','employeeid','id','emp. id'],
  name:  ['name','emp name','employee name','empname','full name','emp. name'],
  doj:   ['doj','date of joining','date of join','joining date','join date','doj_fmt'],
  level: ['level','grade','band'],
  designation: ['designation','title','role','position'],
  location: ['location','city','office','base','work location'],
  mobile: ['mobile','phone','contact','mobile no','mobile number','phone no','contact no','mob','cell'],
  serviceLine: [
    'service line','sl','practice','vertical','bu','business unit',
    'service line & department','service line &amp; department',
    'service line department','service line & dept','department','dept',
    'service line and department','service line enabling','service line ps',
    'service line ims','enabling','inside sales',
  ],
  client: ['client','account','customer','project','client name','client / project'],
  manager: ['manager','reporting manager','reporting to','rm','reports to','line manager','direct manager'],
  suggestedEmail: [
    'suggested email','suggested email id','company email','work email',
    'official email','email id','email','corporate email','mindteck email','office email',
  ],
  personalEmail: ['personal email','personal email id','gmail','yahoo','private email','alternate email','personal mail','personal e-mail'],
  bloodGroup: ['blood group','blood','bg','blood type','bloodgroup','blood grp'],
  dob: ['dob','date of birth','birth date','birthdate','dob_fmt','d.o.b'],
  cif: ['cif'],
};
...
function normalizeDate(s) {
  ...
  if (m) {
    ...
    if (!mm) return str;
    const yy = m[3].length === 2 ? (parseInt(m[3]) > 50 ? `19${m[3]}` : `20${m[3]}`) : m[3];
...
function matchHeader(cell) {
  let c = cell.toLowerCase().trim().replace(/\s+/g,' ');
  c = c.replace(/\s*&\s*(amp;)?\s*(department|dept|sub\w*|division)?\s*$/i,'').trim();
  c = c.replace(/&amp;/g,'&').replace(/\s+/g,' ').trim();
  for (const [field, aliases] of Object.entries(HEADER_MAP)) {
    if (aliases.some(a => c === a || c.includes(a) || a.includes(c))) return field;
  }
  return null;
}
...
function parseExcelPaste(raw) {
  ...
  const HEADER_KEYWORDS = ['name','id','eid','email','date','doj','dob','designation','level','location','mobile','service','department','manager','suggested','personal','blood','cif','client','grade','band','phone','mob'];
  ...
  if (headerLine >= 0 && lines[headerLine+1] && isHeaderRow(lines[headerLine+1])) {
    ...
    const merged = Array.from({length:maxLen},(_,i) => {
      ...
      const h2 = (h2cells[i]||'').replace(/^[\s&]+(?:amp;)?/i,'').trim();
  ...
  const dataLineStr = lines.slice(dataStart).find(line => line.split('\t').filter(c=>c.trim()).length >= 3);
...
// ── AI-powered parser — calls backend /api/chatbot/parse-employee ─────────────
async function parseEmployeeWithAI(rawText) {
  const token = localStorage.getItem('token');
  const res = await fetch(`${API}/chatbot/parse-employee`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
  ...
  if (!res.ok) throw new Error('parse-employee API error ' + res.status);
...
function getMissingFields(parsed) {
  return REQUIRED_EMP_FIELDS.filter(f => !parsed[f.key] || String(parsed[f.key]).trim() === '' || parsed[f.key] === null);
}
  ...
  return (
    <div style={{ marginTop:10, background:'var(--surface,#fff)', border:'1px solid var(--border,#e5e7eb)', borderRadius:12, overflow:'hidden', fontSize:12 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'linear-gradient(135deg,#f8faff,#eff2ff)', borderBottom:'1px solid var(--border,#e5e7eb)' }}>
        <div style={{ width:40, height:40, borderRadius:'50%', flexShrink:0, background: employee ? 'linear-gradient(135deg,#6366f1,#818cf8)' : 'linear-gradient(135deg,#f59e0b,#fbbf24)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:14, color:'#fff' }}>{initials}</div>
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:700, fontSize:13, color:'#111827' }}>{name}</div>
          <div style={{ fontSize:11, color:'#6b7280' }}>{employee ? `${employee.emp_id} · ${employee.designation||'—'}` : 'Found in allocation records'}</div>
        </div>
        {employee && <span style={{ fontSize:10, fontWeight:600, padding:'3px 8px', borderRadius:20, background: employee.status==='Active'?'#dcfce7':'#fee2e2', color: employee.status==='Active'?'#15803d':'#dc2626' }}>{employee.status}</span>}
      </div>
      <div style={{ display:'flex', gap:6, padding:'7px 12px', flexWrap:'wrap', borderBottom:'1px solid #f3f4f6', background:'#fafafa' }}>
        {activeAllocs.length>0 && <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'#eff6ff', color:'#1d4ed8', border:'1px solid #93c5fd', fontWeight:600 }}>💻 {activeAllocs.length} active laptop{activeAllocs.length>1?'s':''}</span>}
        {returnedAllocs.length>0 && <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'#f3f4f6', color:'#374151', border:'1px solid #e5e7eb', fontWeight:600 }}>🔄 {returnedAllocs.length} returned</span>}
        {auditLogs.length>0 && <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'#fef9c3', color:'#854d0e', border:'1px solid #fde047', fontWeight:600 }}>📋 {auditLogs.length} activity log{auditLogs.length>1?'s':''}</span>}
        {accessories.length>0 && <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'#f0fdf4', color:'#15803d', border:'1px solid #86efac', fontWeight:600 }}>🎧 {accessories.length} accessory record{accessories.length>1?'s':''}</span>}
        {acceptanceTokens.length>0 && <span style={{ fontSize:11, padding:'3px 10px', borderRadius:20, background:'#f5f3ff', color:'#5b21b6', border:'1px solid #c4b5fd', fontWeight:600 }}>✅ {acceptanceTokens.length} acceptance record{acceptanceTokens.length>1?'s':''}</span>}
        ...
        <div>
          {employee ? (
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', padding:'8px 12px', gap:'2px 12px' }}>
              {[['Level',employee.level],['Service Line',employee.service_line],['Location',employee.location],['Client',employee.client],['Manager',employee.reporting_manager],['DOJ',fmtDate(employee.doj)],['DOB',fmtDate(employee.dob)],['Blood Group',employee.blood_group],['Mobile',employee.mobile_no]].map(([label,val]) => (
              ...
              {activeAllocs.map((a,i) => (
                <div key={i} style={{ fontSize:11, padding:'5px 8px', borderRadius:6, background:'#eff6ff', color:'#1d4ed8', marginBottom:4, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <div style={{ display:'flex', gap:4 }}>
                    <button onClick={() => onReceive&&onReceive(a,null)} style={{ padding:'3px 8px', borderRadius:12, fontSize:10, fontWeight:600, cursor:'pointer', background:'#dbeafe', border:'1px solid #93c5fd', color:'#1d4ed8' }}>Receive</button>
                    <button onClick={() => onSwap&&onSwap(a)} style={{ padding:'3px 8px', borderRadius:12, fontSize:10, fontWeight:600, cursor:'pointer', background:'#f3f4f6', border:'1px solid #e5e7eb', color:'#374151' }}>Swap</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            return (
              <div key={i} style={{ padding:'8px', borderRadius:8, background: isActive?'#eff6ff':'#f9fafb', border:`1px solid ${isActive?'#93c5fd':'#e5e7eb'}`, marginBottom:8 }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                  ...
                  <span style={{ fontSize:10, fontWeight:600, padding:'2px 7px', borderRadius:20, background: isActive?'#dbeafe':'#f3f4f6', color: isActive?'#1d4ed8':'#6b7280' }}>{a.status}</span>
                </div>
                <div style={{ fontSize:11, color:'#374151', marginBottom:2 }}>{a.brand||''} {a.model||''}{a.department?` · ${a.department}`:''}</div>
                ...
                {isActive && (
                  <div style={{ display:'flex', gap:6 }}>
                    <button onClick={() => onReceive&&onReceive(a,null)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:14, fontSize:11, fontWeight:600, cursor:'pointer', background:'#dbeafe', border:'1px solid #93c5fd', color:'#1d4ed8' }}><ArrowRight size={10}/> Receive Back</button>
                    <button onClick={() => onSwap&&onSwap(a)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:14, fontSize:11, fontWeight:600, cursor:'pointer', background:'#f3f4f6', border:'1px solid #e5e7eb', color:'#374151' }}>Swap</button>
                  </div>
                )}
              </div>
            );
          ...
          {auditLogs.map((log,i) => (
            <div key={i} style={{ padding:'6px 8px', borderRadius:6, background:'#fafafa', border:'1px solid #f3f4f6', marginBottom:6 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8 }}>
                ...
                <div style={{ fontSize:10, color:'#9ca3af', whiteSpace:'nowrap', flexShrink:0 }}>{fmtDate(log.created_at)}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding:'8px 12px' }}>
          ...
          {accessories.map((acc,i) => (
            <div key={i} style={{ padding:'6px 8px', borderRadius:6, background:'#f0fdf4', border:'1px solid #86efac', marginBottom:6 }}>
              ...
              <div style={{ fontSize:11, color:'#374151', marginTop:2 }}>Qty: {acc.quantity} · Status: {acc.status}</div>
              <div style={{ fontSize:10, color:'#6b7280', marginTop:2 }}>
                Date: {fmtDate(acc.allocation_date)}{acc.asset_id?` · Asset: ${acc.asset_id}`:''}{acc.department?` · Dept: ${acc.department}`:''}
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={i} style={{ padding:'6px 8px', borderRadius:6, background: t.status==='accepted'?'#f0fdf4':'#fef9c3', border:`1px solid ${t.status==='accepted'?'#86efac':'#fde047'}`, marginBottom:6 }}>
              <div style={{ display:'flex', justifyContent:'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:'#6b7280', marginTop:2 }}>Status: {t.status}{t.has_damage?' · ⚠️ Damage reported':''}</div>
...
function NewEmployeePreviewCard({ parsed, onConfirm, onEdit, onCancel }) {
  ...
  const fields = [
    ['Employee ID',parsed.empId],['Designation',parsed.designation],['Level',parsed.level],
    ['Location',parsed.location],['Date of Joining',parsed.doj?fmtDate(parsed.doj):null],
    ['Date of Birth',parsed.dob?fmtDate(parsed.dob):null],['Mobile',parsed.mobile],
    ['Service Line',parsed.serviceLine],['Client',parsed.client],['Manager',parsed.manager],
  ...
  return (
    <div style={{ marginTop:10, background:'var(--surface,#fff)', border:'1px solid #86efac', borderRadius:12, overflow:'hidden', fontSize:12 }}>
      ...
      <div style={{ display:'flex', gap:8, padding:'10px 12px', borderTop:'1px solid #f0fdf4', background:'#fafafa', flexWrap:'wrap' }}>
        <button onClick={onConfirm} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'8px 14px', borderRadius:20, fontSize:12, fontWeight:700, cursor:'pointer', background:'linear-gradient(135deg,#22c55e,#16a34a)', border:'none', color:'#fff', boxShadow:'0 2px 8px rgba(34,197,94,0.4)' }}>
          <ArrowRight size={12}/> Open Add Employee Form
        </button>
        <button onClick={onEdit} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 14px', borderRadius:20, fontSize:12, fontWeight:600, cursor:'pointer', background:'#eff2ff', border:'1px solid #c7d2fe', color:'#4338ca' }}>
          <Edit2 size={11}/> Edit &amp; Fill
        </button>
        <button onClick={onCancel} style={{ padding:'8px 12px', borderRadius:20, border:'1px solid #e5e7eb', background:'transparent', fontSize:12, cursor:'pointer', color:'#6b7280' }}>Cancel</button>
      </div>
    </div>
  );
}
      ...
      <div style={{ padding:'10px 12px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px 12px', marginBottom:12 }}>
          {REQUIRED_EMP_FIELDS.map(f => {
            ...
            return (
              <div key={f.key}>
                <label style={{ fontSize:10, color:'#374151', fontWeight:600, display:'block', marginBottom:3 }}>{f.label} <span style={{ color:'#dc2626' }}>*</span></label>
          ...
          {[{key:'suggestedEmail',label:'Company Email',type:'email'},{key:'personalEmail',label:'Personal Email',type:'email'}].map(f => (
            <div key={f.key}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button onClick={onCancel} style={{ padding:'8px 16px', borderRadius:20, border:'1px solid #e5e7eb', background:'transparent', fontSize:12, cursor:'pointer', color:'#6b7280' }}>Back</button>
        </div>
      </div>
    </div>
  );
      ...
      {allocations.map((a,i) => (
        <div key={i} style={{ padding:'8px 12px', borderBottom:'1px solid #f3f4f6' }}>
          ...
          <div style={{ fontSize:11, color:'#6b7280', marginBottom:6 }}>
            {a.brand||''} {a.model||''} · Since {fmtDate(a.allocation_date||a.allocationDate)}{a.department?` · ${a.department}`:''}
          </div>
          <div style={{ display:'flex', gap:6 }}>
            <button onClick={() => onReceive&&onReceive(a)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:14, fontSize:11, fontWeight:600, cursor:'pointer', background:'#dbeafe', border:'1px solid #93c5fd', color:'#1d4ed8' }}><ArrowRight size={10}/> Receive</button>
            <button onClick={() => onSwap&&onSwap(a)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:14, fontSize:11, fontWeight:600, cursor:'pointer', background:'#f3f4f6', border:'1px solid #e5e7eb', color:'#374151' }}>Swap</button>
          </div>
        </div>
      ))}
...
function AssetCard({ asset, allocations, onAllocate, onReceive }) {
  ...
  const statusColor = { Stock:{bg:'#dcfce7',color:'#15803d'}, Allocated:{bg:'#dbeafe',color:'#1d4ed8'}, Repair:{bg:'#fef9c3',color:'#92400e'}, Scrap:{bg:'#fee2e2',color:'#dc2626'} }[asset.status]||{bg:'#f3f4f6',color:'#374151'};
  return (
    <div style={{ marginTop:10, background:'var(--surface,#fff)', border:`1px solid ${isStock?'#86efac':isAllocated?'#93c5fd':'var(--border,#e5e7eb)'}`, borderRadius:12, overflow:'hidden', fontSize:12 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'linear-gradient(135deg,#f8faff,#eff2ff)', borderBottom:'1px solid var(--border,#e5e7eb)' }}>
        <div style={{ width:38, height:38, borderRadius:8, flexShrink:0, background:'#e0e7ff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>💻</div>
        <div style={{ flex:1 }}>
        ...
        <span style={{ fontSize:10, fontWeight:600, padding:'3px 8px', borderRadius:20, background:statusColor.bg, color:statusColor.color }}>{asset.status}</span>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', padding:'8px 12px', gap:'2px 12px' }}>
        {[['Serial No',asset.serial],['Config',asset.config],['Processor',asset.processor],['RAM',asset.ram],['Storage',asset.storage],['Location',asset.location],['Warranty End',fmtDate(asset.warranty_end)]].map(([label,val]) => (
          <div key={label} style={{ padding:'3px 0', borderBottom:'1px solid #f3f4f6' }}>
            <div style={{ fontSize:10, color:'#9ca3af' }}>{label}</div>
            <div style={{ fontWeight:500, color:'#374151' }}>{val||'—'}</div>
          </div>
      ...
      <div style={{ padding:'8px 12px', borderTop:'1px solid #f3f4f6', background:'#fafafa' }}>
        <div style={{ fontSize:10, fontWeight:700, color:'#6366f1', marginBottom:4, textTransform:'uppercase', letterSpacing:'0.06em' }}>Current Allocation</div>
        {alloc
          ? <div style={{ fontSize:11, padding:'4px 8px', borderRadius:6, background:'#eff6ff', color:'#1d4ed8' }}><strong>{alloc.emp_name||alloc.empName}</strong> ({alloc.emp_id||alloc.empId}){alloc.department?` · ${alloc.department}`:''}{' · Since '}{fmtDate(alloc.allocation_date||alloc.allocationDate)}</div>
      ...
      {isStock && (
        <div style={{ padding:'10px 12px', borderTop:'1px solid #86efac', background:'#f0fdf4' }}>
          <div style={{ fontSize:11, color:'#15803d', marginBottom:8 }}>🟢 This laptop is <strong>available in stock</strong>. Would you like to allocate it?</div>
          <button onClick={() => onAllocate&&onAllocate(asset.asset_id||asset.id)} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 16px', borderRadius:20, fontSize:12, fontWeight:700, cursor:'pointer', background:'linear-gradient(135deg,#22c55e,#16a34a)', border:'none', color:'#fff', boxShadow:'0 2px 8px rgba(34,197,94,0.4)' }}>
      ...
      {isAllocated && (
        <div style={{ padding:'10px 12px', borderTop:'1px solid #93c5fd', background:'#eff6ff' }}>
          <div style={{ fontSize:11, color:'#1d4ed8', marginBottom:8 }}>🔵 This laptop is currently with <strong>{alloc?.emp_name||alloc?.empName||'an employee'}</strong>. Would you like to receive it back?</div>
          <button onClick={() => onReceive&&onReceive(alloc,asset)} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 16px', borderRadius:20, fontSize:12, fontWeight:700, cursor:'pointer', background:'linear-gradient(135deg,#3b82f6,#1d4ed8)', border:'none', color:'#fff', boxShadow:'0 2px 8px rgba(59,130,246,0.4)' }}>
    ...
    <div style={{ display:'flex', flexDirection: isUser?'row-reverse':'row', alignItems:'flex-start', gap:8, marginBottom:14 }}>
      ...
      <div style={{ maxWidth:'82%', position:'relative' }}>
        <div style={{ padding:'8px 12px', borderRadius: isUser?'14px 3px 14px 14px':'3px 14px 14px 14px', background: isUser?'linear-gradient(135deg,#6366f1,#818cf8)':'var(--surface,#fff)', border: isUser?'none':'1px solid var(--border,#e5e7eb)', boxShadow: isUser?'0 3px 10px rgba(99,102,241,0.3)':'0 2px 6px rgba(0,0,0,0.05)' }}>
          ...
          {!isUser&&msg.empCard && <EmployeeCard emp={msg.empCard} allocations={toArr(db?.allocations)} licenses={toArr(db?.licenses)}/>}
          {!isUser&&msg.assetCard && <AssetCard asset={msg.assetCard} allocations={toArr(db?.allocations)} onAllocate={onAllocate} onReceive={onReceive}/>}
          {!isUser&&msg.comprehensiveCard && (
            <ComprehensivePersonCard
              ...
              onReceive={onReceive} onSwap={onSwap}
            />
          )}
          ...
          {!isUser&&msg.allocationCards && (
            <AllocationCard allocations={msg.allocationCards} onReceive={(a) => onReceive&&onReceive(a,null)} onSwap={(a) => onSwap&&onSwap(a)}/>
          )}
        ...
        {!isUser && (
          <button onClick={() => { navigator.clipboard.writeText(msg.content); setCopied(true); setTimeout(()=>setCopied(false),1600); }}
            style={{ position:'absolute', top:6, right:-26, background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', opacity:0.45, padding:2, display:'flex', transition:'opacity 0.15s' }}
...
async function think(rawQuery, db, navigate) {
  ...
  // ── 1. Excel paste — AI powered ────────────────────────────────────────────
  if (looksLikeExcelPaste(rawQuery)) {
    let parsed;
    try {
      // PRIMARY: AI parser via backend
      parsed = await parseEmployeeWithAI(rawQuery);
    } catch (aiErr) {
      console.warn('[chatbot] AI parse failed, falling back to regex:', aiErr.message);
      try {
        // FALLBACK: regex parser
        parsed = parseExcelPaste(rawQuery);
      } catch (regexErr) {
        parsed = null;
      }
    }

    if (!parsed || (!parsed.empId && !parsed.name)) {
      return { content: '❌ Could not parse the pasted data. Please copy the **header row + data row** directly from Excel and try again.' };
    }

    // Check if employee already exists
    const match = parsed.empId
      ? employees.find(e => e.emp_id?.toUpperCase() === parsed.empId.toUpperCase())
      : null;

    if (match) {
      return {
        content: `## ✅ Already in System\n\nEmployee **${parsed.empId}** already exists as **${match.emp_name}**.`,
        empCard: match,
      };
    }
  ...
  if (isActiveAllocQuery||isAllocQuery) {
    ...
    if (empMatch) {
      ...
      return { content:`**${empMatch.emp_name}** has ${empAllocs.length} active laptop allocation${empAllocs.length>1?'s':''}:`, empCard:empMatch, allocationCards:empAllocs };
    }
    return { content:`## 💼 Active Allocations (${active.length})\n\nShowing first ${Math.min(active.length,10)}:`, allocationCards:active.slice(0,10) };
  }

  // ── 3. Pending returns ─────────────────────────────────────────────────────
  if (/pending.*return|exited.*laptop|offboard.*laptop/i.test(rawQuery)) {
    const deleted = toArr(db.deleted||[]);
    const deletedWithAlloc = deleted.filter(e => allocations.some(a => (a.emp_id||a.empId)===e.emp_id && (a.status==='Active'||a.status==='active')));
    if (!deletedWithAlloc.length) return { content:'✅ No pending laptop returns — all exited employees have returned their laptops.' };
    return { content:`## ⚠️ Pending Laptop Returns (${deletedWithAlloc.length})\n\n`+deletedWithAlloc.map(e=>`• **${e.emp_name}** (${e.emp_id}) — exited ${e.deleted_at?fmtDate(e.deleted_at):'recently'}`).join('\n') };
  }
  ...
  if (eit.length>0) {
    ...
    if (e) {
      ...
      return { content:`Found **${e.emp_name}** — full details:`, empCard:e, ...(empAllocs.length?{allocationCards:empAllocs}:{}) };
    }
  }
  ...
  if (isNameLike) {
    ...
    const matches = (target) => { if (!target) return false; const t=target.toLowerCase(); return words.every(w => t.includes(w)); };
    const matchedEmployee    = employees.find(e => matches(e.emp_name))||null;
    const matchedAllocs      = toArr(db.allocations).filter(a => matches(a.emp_name||a.empName||''));
    const matchedLogs        = toArr(db.auditLogs||[]).filter(log => matches(log.performed_by||'')||matches(log.detail||'')).slice(0,10);
    const matchedAccessories = toArr(db.accessories).filter(a => matches(a.emp_name||''));
    const matchedTokens      = toArr(db.acceptanceTracker||[]).filter(t => matches(t.emp_name||''));
    if (!matchedEmployee&&matchedAllocs.length===0&&matchedLogs.length===0&&matchedAccessories.length===0&&matchedTokens.length===0) {
      // fall through to AI
    } else {
      const displayName = matchedEmployee?.emp_name||matchedAllocs[0]?.emp_name||matchedAllocs[0]?.empName||matchedAccessories[0]?.emp_name||matchedTokens[0]?.emp_name||rawQuery.trim();
      const parts = [];
      if (matchedEmployee) parts.push('✅ In employee directory'); else parts.push('⚠️ Not in employee directory');
      ...
      return {
        ...
        comprehensiveCard:{ name:displayName, employee:matchedEmployee, allocations:matchedAllocs, auditLogs:matchedLogs, accessories:matchedAccessories, acceptanceTokens:matchedTokens },
      };
    }
  }
...
export default function AIChatbot({ currentPage='default' }) {
  ...
  const [messages, setMessages]   = useState([{
    id:0, role:'assistant',
    content:`## 👋 Hi! I'm AssetOps AI\n\nType an employee name for instant suggestions, paste an Excel row to auto-parse all fields, or ask about allocations!`,
  }]);
  ...
  const handleAllocate = useCallback((assetId) => { setOpen(false); navigate('/allocate',{state:{prefilledAssetId:assetId}}); },[navigate]);
  const handleReceive  = useCallback((alloc,asset) => {
    setOpen(false);
    navigate('/receive',{ state:{ allocationId:alloc?.id||alloc?.dbId||alloc?.alloc_id, assetId:asset?.asset_id||asset?.id||alloc?.asset_id||alloc?.assetId, empId:alloc?.emp_id||alloc?.empId, empName:alloc?.emp_name||alloc?.empName } });
  },[navigate]);
  const handleSwap = useCallback((alloc) => {
    setOpen(false);
    navigate('/swap',{ state:{ prefillAllocationId:alloc?.id||alloc?.dbId||alloc?.alloc_id, prefillAssetId:alloc?.asset_id||alloc?.assetId, prefillEmpId:alloc?.emp_id||alloc?.empId, prefillEmpName:alloc?.emp_name||alloc?.empName } });
  },[navigate]);
  const handleConfirmNewEmployee = useCallback((empData,photoFile) => {
    setOpen(false);
    navigate('/employees',{ state:{ addEmployee:{
      emp_id:empData.empId||'', emp_name:empData.name||'', doj:empData.doj||'', dob:empData.dob||'',
      level:empData.level||'', designation:empData.designation||'', location:empData.location||'',
      mobile_no:empData.mobile||'', service_line:empData.serviceLine||'', client:empData.client||'',
      reporting_manager:empData.manager||'', suggested_email:empData.suggestedEmail||'',
      personal_email:empData.personalEmail||'', blood_group:empData.bloodGroup||'',
      company_email:empData.suggestedEmail||'', notes:empData.cif?`CIF: ${empData.cif}`:'',
    }, photoFile } });
  },[navigate]);
  const handleEditNewEmployee = useCallback((msgId,forceEdit) => {
    setMessages(prev => prev.map(m => m.id!==msgId ? m : { ...m, editMode: forceEdit!==undefined?forceEdit:!m.editMode }));
  },[]);

  const loadDb = useCallback(() => {
    Promise.allSettled([
      apiFetch('/assets'), apiFetch('/employees'), apiFetch('/licenses'),
      apiFetch('/allocations'), apiFetch('/repairs'), apiFetch('/scraps'),
      apiFetch('/employees/deleted'), apiFetch('/network-assets'),
      apiFetch('/accessories'), apiFetch('/acceptance-tracker'),
      apiFetch('/users'), apiFetch('/audit-logs?limit=200'),
      ...
      setDb({
        assets:toArr(assets.value), employees:toArr(employees.value), licenses:toArr(licenses.value),
        allocations:toArr(allocations.value), repairs:toArr(repairs.value), scraps:toArr(scraps.value),
        deleted:deletedArr, networkAssets:toArr(networkAssets.value), accessories:toArr(accessories.value),
        acceptanceTracker:toArr(acceptanceTracker.value), users:toArr(users.value), auditLogs:toArr(auditLogs.value),
      });
    });
  },[]);
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(id)) {
      ...
      s.textContent=`@keyframes aiDot{0%,80%,100%{transform:scale(0.6);opacity:0.4}40%{transform:scale(1);opacity:1}}`;
          ...
          {!minimized && (
            <>
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'12px 12px 4px', scrollbarWidth:'thin', scrollbarColor:'#e5e7eb transparent' }}>
                {messages.map((m,i) => (
                  <Bubble key={i} msg={m} db={db} onAllocate={handleAllocate} onReceive={handleReceive} onSwap={handleSwap} onConfirmNewEmployee={handleConfirmNewEmployee} onEditNewEmployee={handleEditNewEmployee}/>
                ))}
                {loading && <Typing/>}
                <div ref={bottomRef}/>
              </div>
              ...
              <div style={{ padding:'10px 12px', flexShrink:0, borderTop:'1px solid var(--border,#e5e7eb)', background:'var(--surface,#fff)', position:'relative' }}>
                ...
                <div style={{ display:'flex', gap:8, alignItems:'flex-end' }}>
                  <div style={{ flex:1, position:'relative' }}>
                    ...
                    <textarea ref={inputRef} value={input} onChange={handleInputChange} onKeyDown={handleKey}
                      ...
                      placeholder="Type name, ID, or paste Excel row…" rows={1}
                ...
                <div style={{ fontSize:10, color:'#9ca3af', marginTop:4 }}>Live DB · Type a name to search · Paste Excel row for AI auto-parse</div>
              </div>
            </>
          )}
        </div>
